...
**Latest:** p349 (2026-08-30) — الترحيل من الأنظمة القديمة (فكرة المالك للعمل المتوازي): محوّل rlynx/BDV10 (Access .dblx) كمنتج كامل — خدمة services/legacy_import/bdv_service.py (نقل أمين لمنطق p159: mdbtools←58 جدولاً، ماسترز+حركات مع ديون FIFO وأرصدة افتتاحية، تحقق 20 فحصاً)، API في routes/migration_routes.py (رفع 300MB + مهمة خلفية بحالة في قاعدة المستأجر + تراجع كامل)، معالج /legacy-migration في الواجهة + عنصر قائمة. بنية: mdbtools في Dockerfile + volume legacy_uploads + nginx 100MB. E2E بالملف الحقيقي 46MB: 20/20 ✓ + تراجع كامل + residue=0. Chain: …→p339→p340…→p347→p348→p349.
