...
> **للمحادثة الجديدة: اقرأ هذا الملف كاملاً أولاً، ثم آخر 5 إدخالات في DEPLOYMENT_LOG.md، ثم قل «اكمل» لمواصلة p202.**
> آخر تحديث: 2026-08-20 بعد إتمام p201 — ثغرة تعديل المصروف مسدودة (حدث expense.updated + قيد تسوية + معاملة ذرّية لمسار PUT).
...
## 6) الخطوة التالية — p202: سد ثغرات التغطية المحاسبية (2/5) — دفعات عقود التأجير

**الدافع**: `POST /api/rentals/contracts/{id}/payment` يحرّك الصندوق بلا قيد يومية.

**الخطة**:
1. افحص مسار الدفع الحالي؛ غلّفه بمعاملة + حدث outbox `rental.payment_received` (payment_id فريد + المبلغ + الصندوق + العقد/العميل).
2. معالج + دالة accounting_auto بنمط p195: مدين الصندوق / دائن 411 (أو حساب إيراد تأجير إن كانت الدفعة لعقد جديد — افحص نموذج البيانات أولاً وقرّر بناءً عليه)؛ منع تكرار.
3. اختبار curl موسوم (عقد تجريبي) + تنظيف دقيق بلا مساس بالبيانات الحقيقية.
4. الثغرات التالية بنفس النمط (كل واحدة مرحلة): الدفعات المسبقة للموردين ← سداد ائتمان المحفظة ← سجلات الدفع اليدوية.
...
- النسخ الاحتياطية: /opt/ntcommerce/backups/p182..p201
