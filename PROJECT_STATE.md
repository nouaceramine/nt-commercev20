...
> **للمحادثة الجديدة: اقرأ هذا الملف كاملاً أولاً، ثم آخر 5 إدخالات في DEPLOYMENT_LOG.md، ثم قل «اكمل» لمواصلة p208.**
> آخر تحديث: 2026-08-20 بعد إتمام p207 — الملخص الضريبي من سطور اليومية (بديل حساب المستندات الخاطئ: كان يجمع كل مبيعات السنة ويصفّر المصاريف).
...
## 6) الخطوة التالية — p208: واجهة القيد اليدوي

**الدافع**: `POST /api/accounting/journal-entries` موجود بلا واجهة — المحاسب يحتاج إدخال قيود يدوية (تسويات، مصاريف غير موثقة...).

**الخطة**:
1. نموذج في AccountingPage بنفس مكوّنات التصميم: تاريخ + وصف + مرجع + سطور ديناميكية (حساب من قائمة /accounts + مدين + دائن) مع تحقق توازن حي قبل الإرسال؛ القيد يُولد pending ثم زر اعتماد (approve موجود في الخلفية PUT .../approve).
2. جدول القيود الحالي في الصفحة يعرض القيود اليدوية بحالتها؛ اعتماد قيد pending يجب أن يحدّث مرايا الأرصدة — افحص approve أولاً (قد لا يحدّث المرايا! أصلحه إن لزم ضمن p208).
3. اختبار curl + واجهة: قيد يدوي غير متوازن ← 400 ← متوازن pending ← اعتماد ← يظهر في الميزان/الدفتر ← توثيق والتزام.
4. بعده p209: قفل السنة المالية.
