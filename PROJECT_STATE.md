...
> **للمحادثة الجديدة: اقرأ هذا الملف كاملاً أولاً، ثم آخر 5 إدخالات في DEPLOYMENT_LOG.md، ثم قل «اكمل» لمواصلة p204.**
> آخر تحديث: 2026-08-20 بعد إتمام p203 — الدفعات المسبقة للموردين: أصبحت تُخصم فعلياً من الصندوق + قيد (مدين 402 سلف الموردين / دائن الصندوق) + مسار ذرّي.
...
## 6) الخطوة التالية — p204: سد ثغرات التغطية المحاسبية (4/5) — سداد ائتمان المحفظة والأقساط

**الدافع**: `/wallet/settle-credit` والأقساط تحرّك مالاً بلا قيود يومية.

**الخطة**:
1. افحص مسارات المحفظة (settle-credit + الأقساط) ونموذج بياناتها أولاً؛ حدّد المعالجة (الصندوق مقابل 411 أو حساب ائتمان محفظة).
2. غلّف بمعاملة + حدث outbox (id فريد) + معالج + دالة accounting_auto بنمط p195 مع منع تكرار.
3. اختبار curl موسوم + تنظيف دقيق.
4. ثم الأخيرة: سجلات الدفع اليدوية /payments/records ← ثم ثغرات التأجير المجاورة (الدفعة الأولى، الوديعة، الإغلاق).
...
- النسخ الاحتياطية: /opt/ntcommerce/backups/p182..p203
