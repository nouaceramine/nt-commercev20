...
> **للمحادثة الجديدة: اقرأ هذا الملف كاملاً أولاً، ثم آخر 5 إدخالات في DEPLOYMENT_LOG.md، ثم قل «اكمل» لمواصلة p209.**
> آخر تحديث: 2026-08-21 بعد إتمام p208 — واجهة القيد اليدوي (حوار بسطور ديناميكية وتحقق توازن حي + زر اعتماد للمعلّقة؛ الخلفية لم تُمس).
...
## 6) الخطوة التالية — p209: قفل السنة المالية

**الدافع**: إقفال الفترة — ترحيل نتيجتها (7xx − 6xx) إلى رأس المال 101 وتصفير حسابات الإيرادات/المصاريف حتى لا تختلط فترتان، مع منع تعديل قيود فترة مقفلة.

**الخطة**:
1. POST /accounting/fiscal-close {year} ← قيد إقفال موسوم source_tag=fiscal_close (إيرادات سالبة ← مدين 7xx / دائن 101؛ والعكس للخسارة) بمنع تكرار لكل سنة؛ GET /accounting/fiscal-close يعيد السنوات المقفلة. فحص: صافي 7xx/6xx للسنة = 0 بعد القيد، والميزان يبقى متوازناً.
2. منع الإنشاء/الاعتماد لقيد تاريخه داخل سنة مقفلة (403) — في create_journal_entry وapprove.
3. واجهة /accounting: بطاقة قفل السنة (معاينة النتيجة + زر ترحيل مرة واحدة) بنمط بطاقة الافتتاحي.
4. اختبار curl موسوم ثم تنظيف دقيق ← توثيق والتزام. بعده المسار 3 (تحصينات: DLQ، أرشفة outbox، توحيد SmartReporterAgent).
...
- **p209**: قفل السنة المالية — ترحيل نتيجة الفترة إلى رأس المال وتصفير 7xx/6xx + منع تعديل فترة مقفلة
