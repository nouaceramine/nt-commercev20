...
> **للمحادثة الجديدة: اقرأ هذا الملف كاملاً أولاً، ثم آخر 5 إدخالات في DEPLOYMENT_LOG.md، ثم قل «اكمل» لمواصلة p203.**
> آخر تحديث: 2026-08-20 بعد إتمام p202 — دفعات التأجير مغطاة محاسبياً (rental.payment_received ← مدين الصندوق/دائن 701 إيرادات التأجير + مسار الدفع ذرّي).
...
## 6) الخطوة التالية — p203: سد ثغرات التغطية المحاسبية (3/5) — الدفعات المسبقة للموردين

**الدافع**: `POST /api/suppliers/{id}/advance-payment` يحرّك الصندوق بلا قيد يومية.

**الخطة**:
1. افحص مسار الدفعة المسبقة الحالي (نموذج البيانات: هل تُستهلك لاحقاً ضد مشتريات؟)؛ غلّفه بمعاملة + حدث outbox (payment_id فريد + المبلغ + الصندوق + المورد).
2. معالج + دالة accounting_auto بنمط p195: مدين 401 (تخفيض التزام/دفعة مقدمة) / دائن الصندوق — أو حساب سلف إن دلّ النموذج على ذلك؛ منع تكرار.
3. اختبار curl موسوم + تنظيف دقيق بلا مساس بالبيانات الحقيقية.
4. الثغرات التالية: سداد ائتمان المحفظة ← سجلات الدفع اليدوية ← **ثغرات التأجير المجاورة المكتشفة في p202** (الدفعة الأولى عند الإنشاء، الوديعة held/refund، الإغلاق: رسوم تأخير + تحويل المتبقي لدين).
...
- النسخ الاحتياطية: /opt/ntcommerce/backups/p182..p202
