...
**Latest:** p294 (2026-08-24) — نظام عمال المتجر الإلكتروني كامل: دخول هاتف+PIN بمساحة محدودة (قائمة بيضاء خادمية)، إسناد المنفِّذ لكل طلب، عمولات ثابت+نسبة على المُسلَّم مع تصفية دفترية؛ + إصلاح ثغرة تصعيد صلاحيات في get_current_user. Previous: p292 (توحيد المفاتيح)، p291 (تحويل الحصة للخزنة). Chain: …→p290→p291→p292→p293→p294.
