...
**Latest:** p303 (2026-08-26) — ربط البيع بالوصفة (خطة المطاعم أ 1/3): البيع يخصم مكوّنات الوصفة من المخزون ويكلّف السطر بها → COGS (600) وأرباح حقيقية للأطباق؛ الإرجاع/الحذف يستعيدان بدقة عبر recipe_consumption موقّعة. Previous: p302. Chain: …→p301→p302→p303.
...
- **الخطة أ — تصحيح الربحية:** ~~p303 ربط البيع بالوصفة (خصم مكونات + COGS حقيقي + استعادة دقيقة)~~ ✅ ← p304 تكلفة حية بالمتوسط الموزون عند كل شراء + هامش ربح الطبق ← p305 تقرير Food Cost % + ربحية الأطباق + الهالك
- **الخطة ب — واجهة متخصصة:** شاشة مطبخ KDS (SSE جاهز) + إضافات/بدائل (modifiers) + وجبات Combo + مخطط طاولات بصري + تقسيم الفاتورة
- **الخطة ج — منظومة متكاملة:** QR على الطاولة + تطبيق نادل + شاشة انتظار + تكامل توصيل (Yassir) + تنبؤ AI بمبيعات الأطباق ← قائمة مشتريات تلقائية + إشعار WhatsApp عند الجاهزية
