...
> **للمحادثة الجديدة: اقرأ هذا الملف كاملاً أولاً، ثم آخر 5 إدخالات في DEPLOYMENT_LOG.md، ثم قل «اكمل» لمواصلة p207.**
> آخر تحديث: 2026-08-20 بعد إتمام p206 — دورة التأجير كاملة محاسبياً (وديعة 203 + دفعة أولى + إغلاق بقيد 411/701) + إصلاح سباق entry_number في _insert_entry. (p205 = محرك عمولات المنصة، جلسة موازية ef11a5c.)
...
## 6) الخطوة التالية — p207: التقارير الضريبية من سطور اليومية

**الدافع**: حساب tax-summary القديم يعتمد مستندات لا سطور اليومية (مثل ميزان ما قبل p196).

**الخطة**:
1. استبدل `GET /api/accounting/reports/tax-summary` ليُحسب من سطور اليومية في النافذة مع الإبقاء على شكل الاستجابة المتعاقد عليه مع الواجهة.
2. اختبار curl بمقارنة الأرقام قبل/بعد + توثيق والتزام.
3. بعده p208: واجهة القيد اليدوي (POST /accounting/journal-entries بلا واجهة) ثم p209: قفل السنة المالية.
...
- النسخ الاحتياطية: /opt/ntcommerce/backups/p182..p206
