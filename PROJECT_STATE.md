...
**Latest:** p286 (2026-08-24) — spare-parts families: is_spare_parts flag on product families; maintenance stock (/repairs/parts + ticket part picker) now shows only flagged-family products + legacy part_category docs; products inventory unchanged. Previous: p285 (dashboard crash fix). — instant shipping webhooks: public per-token receiver + per-courier normalisers (Yalidine/Guepex/ZR/Maystro/Ecotrack), UI dialog with copy/rotate; polling remains for the rest. Chain: …→p283→p284→p285→p286.

> **للمحادثة الجديدة: اقرأ هذا الملف كاملاً أولاً، ثم آخر 5 إدخالات في DEPLOYMENT_LOG.md.**
> آخر تحديث: 2026-08-21 بعد p223–p228 (التقرير الاستشاري: قواعد الهامش، فوترة AI، تصنيف أخطاء AutoHeal، تشفير الأسرار AES-256-GCM، السوق الموحد — النشر، إصلاح expense_number) وp232 (فجوة تراجع البيع الآجل) وp233 (قياس صور AI) وp234 (تحقق DLQ/أرشفة + أول replay ناجح) وp235 (كشف الحالات الشاذة — المستوى 2) وp236 (إنذارات تنبؤية — المستوى 3) وp237–p238 (السوق الموحد: الطلبات العابرة + التسويات) وp239 (قيد تحصيل ecom أصبح مسجلاً دفترياً). **المسار 1 أُغلق تحققياً في p232؛ المسارات 1/3/4 كلها مُغلقة (المسار 3: DLQ/أرشفة في p210–p212 وتحققها p234؛ self-healing مستوى 2/3 في p235/p236؛ مسار 4 في p227/p237/p238) — المتبقي: مسار 2 (بنود المالك) فقط. **كل المهام البرمجية المخططة مكتملة؛ المتبقي بنود المالك فقط.** → p240 ✅ كشف المكررات (duplicate detection) + إصلاح فهرس ecom_leads → p241 ✅ SMS حالات التوصيل + رصيد SMS → p242 ✅ إسناد مركز الاتصال → p243 ✅ رفع طلبات Excel/CSV بالجملة → p244 ✅ صفحة التتبع العامة /track → p245 ✅ برنامج الإحالات → p246 ✅ تذاكر الدعم → p247 ✅ واجهة السائق /driver/:token → p251 ✅ ويب هوك الاستقبال (YouCan/LightFunnels/Sheets/مخصص) → p248 ✅ إطار مزامنة الناقلين العام (6 ناقلين، mock+generic_http) → p249 ✅ صندوق الوارد الاجتماعي + التحويل لطلب → p250 ✅ تعدد المتاجر. → p252 ✅ شاشتا السائقين والإحالات في لوحة التحكم → p253 ✅ شاشة الوارد الاجتماعي → p254 ✅ شاشتا تذاكر الدعم (مستأجر + منصة) → p255 ✅ شاشتا مصادر الاستقبال والمتاجر الفرعية + بطاقة مزامنة الناقلين → p256 ✅ توسيع سجل شركات الشحن الجزائرية إلى 78 شركة عبر كل النظام → p257 ✅ محرك الأكواد الذري (لا تكرار تحت التزامن) + فهارس فريدة لكل الكيانات → p258 ✅ أكواد عامة مختومة بالمستأجر (WEB-NT4-/MP-NT4-/ECO-NT4-) + تتبع عام O(1) بالختم مع توافق كامل للأكواد القديمة → p259 ✅ هوية مركبة لمنتجات السوق (MPR-NTx-NNNNN) + طلب بالكود + فهرس فريد → p260 ✅ بحث شامل بكل الأكواد (19 نوع كيان) + تفعيل /search/global (كان ميتاً) + واجهة Ctrl+K مجمعة → p261 ✅ باركود Code128 + QR على الوصولات والفواتير (QR التتبع العام لطلبات ecom) **→ p262 ✅ بريد: مزوِّد واحد يكفي (واجهة صادقة) → p263 ✅ إصلاح الشريط الجانبي للمورد + شارات 23 نشاطاً → p264 ✅ إدارة الميزات في الخطط (إصلاح فقدان صامت) + توحيد مفاتيح الأنشطة retail/wholesale. → p265 ✅ 23 حساباً تجريبياً دائماً (NT-0007..NT-0029) بكل الأنشطة + شارة «تجريبي دائم». → p266 ✅ أكواد ذرية للمحافظ (WL)/الطلبات (WR)/الوكلاء (AG)/المدفوعات (PAY/SP) + فهارس فريدة + /saas/id-audit. → p267 ✅ تبويب «عمولات المنصة» في مالية المنصة + أكواد PCOM ذرية. → p268 ✅ متصفح بيانات المشتركين (قراءة فقط + سجل وصول). → p269 ✅ انتحال الوكلاء + إعادة تعيين كلمات المرور + نوعا أحداث جديدان في الخط الزمني. → p270 ✅ تدقيق القائمة: +/features + مزامنة محرر الترتيب (+19 عنصراً، +2 قسم، −9 ميتة). → p271 ✅ AutoHeal: موقع العطل الدقيق (وحدة/مجموعة/مستأجر/سجلات) في كل تنبيه + عرضه في الواجهة. → p272 ✅ تعميم تشفير AES-256-GCM على 10 مخازن أسرار + ترحيل فعلي (plaintext=0) + بطاقة حالة. → p273 ✅ أداء: كاش Redis (5-6×) + إبطال عند البيع + فهرس expenses.date + تقسيم الحزمة 5.4MB→452KB. → p274 ✅ معالج ربط القنوات الاجتماعية + فحص Meta حقيقي. → p275 ✅ وثيقة خطة الجوال/التابلت → p276 ✅ الجوال أ+ب: شريط تنقل سفلي + ResponsiveTable (سجل المبيعات + صندوق الطلبات) + إصلاح تمرير أفقي قديم في ecom-hub (flex-wrap). → p277 ✅ الجوال ج: POS لمسي (أهداف ≥44/48px + تذييل دفع لاصق على الجوال). → p278 ✅ الجوال د: master-detail للتابلت (تذاكر الدعم + الوارد الاجتماعي) عبر useMediaQuery. → p279 ✅ الجوال هـ: حوارات الجوال + خط حقول 16px + inputmode — **اكتملت خطة الجوال 5/5**. → p280 ✅ تحديث لحظي موسّع: حدث ecom_order.created من 6 مواضع + اشتراكات حية في سجل المبيعات/الزبائن/الموردين/صندوق الطلبات (مثبت E2E). برنامج فجوات المنافسين مكتمل 12/12** (المتبقي: مفاتيح SMS/Meta/الناقلين — بنود المالك)
