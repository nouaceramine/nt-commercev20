...
> **للمحادثة الجديدة: اقرأ هذا الملف كاملاً أولاً، ثم آخر 5 إدخالات في DEPLOYMENT_LOG.md، ثم قل «اكمل» لمواصلة p201.**
> آخر تحديث: 2026-08-20 بعد إتمام p200 — دفتر الأستاذ العام حي (ledger/{account_code} + بطاقة تفصيل برصيد جارٍ في /accounting). p199 نُفّذت بجلسة موازية ورُوجعت وثُبّتت.
...
## 6) الخطوة التالية — p201: سد ثغرات التغطية المحاسبية (1/5) — تعديل المصروف

**الدافع**: `PUT /api/expenses/{id}` يعدّل المصروف لكنه يترك قيد p193 القديم بلا تحديث — اليومية تنحرف عن الواقع.

**الخطة**:
1. افحص مسار تعديل المصروف الحالي؛ عند تغيّر المبلغ/الصندوق والمصروف له قيد مرحّل: غلّف التعديل بمعاملة + حدث outbox `expense.updated` بحمولة (expense_id, old_amount, new_amount, old/new payment_method, currency).
2. معالج في event_consumers + دالة في accounting_auto: قيد تسوية بالفرق (أو عكسي+جديد إن تغيّر الصندوق) بعلامة source_tag="expense_adjustment" ومنع تكرار.
3. اختبار curl موسوم: مصروف تجريبي ← تعديله ← قيد تسوية متوازن والميزان يعكسه ← تنظيف دقيق بلا مساس بالبيانات الحقيقية.
4. الثغرات التالية بنفس النمط (كل واحدة مرحلة): دفعات التأجير ← الدفعات المسبقة للموردين ← سداد ائتمان المحفظة ← سجلات الدفع اليدوية.
...
- النسخ الاحتياطية: /opt/ntcommerce/backups/p182..p200
