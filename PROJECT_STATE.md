...
> **للمحادثة الجديدة: اقرأ هذا الملف كاملاً أولاً، ثم آخر 5 إدخالات في DEPLOYMENT_LOG.md، ثم قل «اكمل» لمواصلة p205.**
> آخر تحديث: 2026-08-20 بعد إتمام p204 — دفع الأقساط مغطّى (installment.paid ← مدين الصندوق/دائن 411 + مسار ذرّي)؛ ثغرتا settle-credit وpayments/records حُسمتا: منصويتان لا تخصان دفاتر المستأجر.
...
## 6) الخطوة التالية — p206: ثغرات التأجير المجاورة (اكتُشفت في p202)

**الدافع**: مسارات تأجير تحرّك مالاً بلا قيود بعد: الدفعة الأولى عند إنشاء العقد (rental_payment) والوديعة (rental_deposit: held ثم returned/kept) ومسار الإغلاق (رسوم تأخير + تحويل المتبقي لدين عميل في المرآة فقط).

**الخطة**:
1. إنشاء العقد: الدفعة الأولى ← قيد (الصندوق/701) عبر حدث rental.payment_received نفسه (توحيداً)؛ الوديعة المحتجزة ← مدين الصندوق / دائن حساب جديد 203 «ودائع أمانات» (liability).
2. الإغلاق: استرجاع الوديعة ← عكسها (203/الصندوق)؛ إبقاؤها (kept) ← 203 ← 701؛ المتبقي المؤجَّل لدين ← مدين 411 / دائن 701 (إيراد مستحق غير محصّل)؛ رسوم التأخير ضمنه.
3. معاملة ذرّية + منع تكرار لكل حدث؛ اختبار curl موسوم كامل الدورة + تنظيف دقيق.
4. بعدها: p207 التقارير الضريبية من سطور اليومية (p201 في الخارطة القديمة)، ثم واجهة القيد اليدوي، ثم قفل السنة.
...
- **p202**: واجهة القيد اليدوي (مسار POST /accounting/journal-entries موجود بلا واجهة)
- **p203**: قفل السنة المالية — ترحيل نتيجة الفترة إلى رأس المال وتصفير 7xx/6xx
...
- مراقبة/تنبيه DLQ (الأحداث الفاشلة بعد 3 محاولات) | أرشفة دورية للـ outbox المنشور | توحيد تقارير SmartReporterAgent القديمة مع تقارير اليومية

### المسار 4: السوق الموحّد (مشروع مستقل مؤجل — لا يبدأ قبل المسار 1 وجلسة تخطيط)
3 مراحل: عرض موحد لمنتجات المستأجرين ← طلبات عابرة للمستأجرين ← تسويات بينهم
...
- النسخ الاحتياطية: /opt/ntcommerce/backups/p182..p205
