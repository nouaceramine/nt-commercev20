...
> **للمحادثة الجديدة: اقرأ هذا الملف كاملاً أولاً، ثم آخر 5 إدخالات في DEPLOYMENT_LOG.md، ثم قل «اكمل» لمواصلة أرشفة outbox (ثم توحيد SmartReporterAgent).**
> آخر تحديث: 2026-08-21 بعد إتمام p210 — تنبيه DLQ الاستباقي (platform_alerts + /admin/event-bus/alerts + ack + بطاقة تنبيهات في لوحة ناقل الأحداث + replay يُغلق التنبيه).
...
2. **أرشفة دورية للـ outbox المنشور (التالي — p211)**: أحداث published القديمة تنقل إلى outbox_archive (تُبقي الجدول خفيفاً) — مهمة دورية في backend + حد زمني (مثلاً 30 يوماً). افحص services/outbox.py أولاً (بنية الحقول published/published_at) ثم أضف حلقة أرشفة بجانب relay + اختبر بأحداث موسومة.
3. **توحيد تقارير SmartReporterAgent القديمة مع تقارير اليومية (p212)**: قرر لكل تقرير قديم: يُستبدل بمسار journal_lines أو يُوسوم legacy.
