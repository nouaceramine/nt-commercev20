...
**Latest:** p348 (2026-08-29) — خطة معالجة الأخطاء الشاملة: 8 أصناف إصلاحات حية (KeyError لوحة الكتالوج، external_id فارغ يصطدم بالفهرس الفريد في leads/webhooks، أكواد جلسات يومية ذرّية مع التفاف DuplicateKeyError، إعادة معاملة البيع كاملة عند تكرار الكود، مهلة watchdog 8→25ث، تصالح 3 فهارس عند الإقلاع) + **جذر عميق فات p347**: 10 روبوتات + 6 خدمات كانت تشتق tenant_<uuid> مباشرة فأعادت إنشاء 28 قاعدة legacy شبحية فارغة وحسبت تقارير عمياء — عولجت بـis_tenant_db/resolve_db_name في 12 ملفاً (migrations وbackups كانا يهجران nt_*!) وأُسقطت الـ28 بعد تحقق المحتوى + أدلة oplog: صفر إعادة إنشاء. تطهير السجل: 17 نتيجة autoheal محلولة بملاحظات + system_errors 8→0. تحليل شامل: 34 قاعدة بلا أيتام، صفر أكواد مكررة، صفر backlogs، حمل 0.06/قرص 42%/RAM 4.5GB، p341/p342 بلا انحدار. Chain: …→p339→p340…→p347→p348.
...
- رمز بوت Telegram (BotFather) | ~~نسخ احتياطي خارجي~~ ✅ **مفعّل (p327)**: rclone→B2 بمفتاح محدود النطاق + أول نسخة مرفوعة وفك تشفيرها مُثبت؛ يوميًا 04:30 تلقائيًا — **يبقى على المالك فقط: نسخ `.backup_key` وحفظه خارج الخادم** | ~~رمز Cloudflare API~~ ✅ **مفعّل (p328)**: نطاق فرعي تلقائي لكل متجر + إدارة مشفرة من /saas/cloudflare-settings | ~~البريد~~ ✅ **مفعّل (p326)**: Resend بمفتاح مشفر + نطاق nt-commerce.net موثق — delivered مثبت
...
### المسار 5: واجهة نشاط المطاعم/البيتزيريا/الأكل السريع (قرار المالك 2026-08-26)
- **الخطة أ — تصحيح الربحية:** ~~p303 ربط البيع بالوصفة~~ ✅ ← ~~p304 تكلفة حية~~ ✅ ← ~~p305 تقرير Food Cost + الهالك (قيد 610/380) + تبويب «تكلفة الطعام»~~ ✅ — **الخطة أ مكتملة**
- **الخطة ب — واجهة متخصصة:** ~~شاشة مطبخ KDS~~ ✅ (p306، منشورة) + ~~إضافات/بدائل (modifiers)~~ ✅ (p308) + ~~وجبات Combo~~ ✅ (p309) + ~~مخطط طاولات بصري + تقسيم الفاتورة~~ ✅ (p310)
- **الخطة ج — منظومة متكاملة:** ~~QR على الطاولة~~ ✅ (p311) + ~~تطبيق نادل~~ ✅ (p312) + ~~شاشة انتظار~~ ✅ (p314) + ~~توصيل~~ ✅ (p316، طلبات توصيل داخلية بسائقين — Yassir لا يوفر API عامًا لحقن طلبات المطاعم) + ~~تنبؤ AI بمبيعات الأطباق ← قائمة مشتريات تلقائية~~ ✅ (p313) + ~~إشعار WhatsApp عند الجاهزية~~ ✅ (p315)
