...
> **للمحادثة الجديدة: اقرأ هذا الملف كاملاً أولاً، ثم آخر 5 إدخالات في DEPLOYMENT_LOG.md، ثم قل «اكمل» لمواصلة تحصينات المسار 3.**
> آخر تحديث: 2026-08-21 بعد إتمام p209 — قفل السنة المالية (قيد إقفال إلى 101 + تصفير 6xx/7xx + منع قيود/اعتماد داخل سنة مقفلة 403 + بطاقة في /accounting). **المسار 1 (الدورة المحاسبية) مكتمل بالكامل.**
...
1. **مراقبة/تنبيه DLQ**: الأحداث الفاشلة بعد 3 محاولات تهبط في dead-letter — تحتاج: عدّاداً ظاهراً (لوحة super-admin أو /api/events/dlq) + تنبيهاً (سجل audit/SSE للمالك) + زر إعادة تشغيل. افحص relay/consumer أولاً: أين تُسجَّل الفاشلة فعلياً؟
2. **أرشفة دورية للـ outbox المنشور**: أحداث published القديمة تنقل إلى outbox_archive (تُبقي الجدول خفيفاً) — مهمة دورية في backend + حد زمني (مثلاً 30 يوماً).
3. **توحيد تقارير SmartReporterAgent القديمة مع تقارير اليومية**: قرر لكل تقرير قديم: يُستبدل بمسار journal_lines أو يُوسوم legacy.
