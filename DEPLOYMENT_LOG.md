## 2026-09-25 — p407: جولة انحدار شاملة بعد دفعة p404–p406

- **30 جناحًا** على الإصدار المنشور 20260925_225414: p365 (21) + p366 (51) + p367 (28) + p368 (17) + p369 (25) + p370 (27) + p371 (26) + p372 (36) + p373 (10) + p374 (23) + p377 (20) + p378 (13) + p379 (18) + p380 (24) + p382 (18) + p385 (21) + p388 (19) + p389 (18) + p390 (17) + p392 (20) + p393 (13) + p394 (18) + p396 (19) + p397 (16) + p400 (18) + p401 (27) + p402 (14) + p404 (22) + p405 (24) + p406 (27) = **650/650 ✅**، صفر انحدار.
- **فحوص النظام**: صحة الباك-إند والواجهة 200، Redis PONG، صفر أسطر [ERROR] في آخر 30 دقيقة، صفر مخلفات مستأجرين تجريبيين، وصفر أحداث outbox متبقية من الاختبارات.
- بهذا تكتمل عائلة تصدير CSV لكل التقارير: الإقفال اليومي Z (p401) + الفترة المخصصة (p394) + الندل (p397) + الطاولات (p404) + المطبخ (p405) + الحجوزات (p406).

## 2026-09-25 — p406: تصدير تقرير الحجوزات CSV

**الجديد:** نقطة `GET /api/restaurant/reservations/report.csv?date_from=…&date_to=…` بنفس استعلام وتجميع تقرير PDF (p388): ملخص (الفترة/إجمالي الحجوزات/مؤكد/حضر/ملغى/لم يحضر/نسبة عدم الحضور/إجمالي الضيوف) + قائمة تفصيلية (التاريخ/الوقت/الزبون/الطاولة/الأشخاص/الحالة بتسميات عربية) بترميز UTF-8 مع BOM لـ Excel. في الواجهة: زر `resv-csv-btn` بجانب زر PDF في بطاقة الحجوزات.

**الملفات:** `backend/routes/restaurant_routes.py` + `frontend/src/pages/TablesMapPage.js`.

**الاختبار:** E2E من 27 نقطة على مستأجر TEST-P406: ثلاث حجوزات (مؤكد/حضر/ملغى، 9 ضيوف) → CSV بـ BOM وCRLF وترويسة اسم الفترة → كل صفوف الملخص والتفاصيل بالتسميات العربية → فترة معكوسة 400 → بلا مصادقة 401 → تنظيف كامل residue=0. **27/27 ✅**

**النشر:** release `20260925_225414` — تحقق `resv-csv-btn` في الحزمة؛ الصحة 200 ✅.

## 2026-09-25 — p405: تصدير أداء المطبخ CSV

**الجديد:** نقطة `GET /api/restaurant/kitchen-stats.csv?days=N` تستدعي `kitchen_stats` داخليًا (صفر ازدواجية) وتُنتج CSV بترميز UTF-8 مع BOM: ملخص (عدد المقدَّمة/متوسط الانتظار/التحضير/الإجمالي/المتأخرة ≥15د) + قسم الأطباق الأبطأ + التوزيع الساعي بمتوسط الزمن. في الواجهة: زر `kds-stats-csv` بجانب زر PDF في شريط إحصاءات شاشة المطبخ (نفس فترة اليوم).

**الملفات:** `backend/routes/restaurant_routes.py` + `frontend/src/pages/KitchenDisplayPage.js`.

**الاختبار:** E2E من 24 نقطة على مستأجر TEST-P405: طلبان بدورة كاملة (pending→preparing→served) → CSV بـ BOM وCRLF وترويسة `kitchen-stats-7d.csv` → صفوف الملخص (2 مقدَّم، 0 متأخر) والأطباق والساعات → تطابق JSON → بلا مصادقة 401 → تنظيف كامل residue=0. **24/24 ✅**

**النشر:** release `20260925_224618` — تحقق `kds-stats-csv` في الحزمة؛ الصحة 200 ✅.

## 2026-09-25 — p404: تصدير إحصاءات الطاولات CSV

**الجديد:** نقطة `GET /api/restaurant/table-stats.csv?days=N` تستدعي `table_stats` داخليًا (صفر ازدواجية) وتُنتج CSV بترميز UTF-8 مع BOM: صف لكل طاولة (الاسم/المقاعد/الطلبات/طلبات-في-اليوم/متوسط الفاتورة/متوسط المدة/الإيراد) مرتّبة تنازليًا بالإيراد — بما فيها الطاولات الميتة بأصفار — ثم صف الإجمالي. في الواجهة: زر `tstats-csv-btn` بجانب زر PDF في بطاقة «دوران الطاولات» (نفس فترة `tstatsDays`).

**الملفات:** `backend/routes/restaurant_routes.py` + `frontend/src/pages/TablesMapPage.js`.

**الاختبار:** E2E من 22 نقطة على مستأجر TEST-P404: ثلاث طاولات (ط1 مدفوعة 800 كاش، ط2 مدفوعة 600 بطاقة، ط3 ميتة) → CSV بـ BOM وCRLF وترويسة `table-stats-7d.csv` → الترتيب التنازلي → أرقام الصفوف وتصفير الميتة → صف الإجمالي (2 طلب / 1400) → تطابق JSON → بلا مصادقة 401 → تنظيف كامل residue=0. **22/22 ✅**

**النشر:** release `20260925_223848` — تحقق `tstats-csv-btn` في الحزمة؛ الصحة 200 ✅.

## 2026-09-21 — p403: جولة انحدار شاملة بعد دفعة p400–p402

- **27 جناحًا** على الإصدار المنشور 20260921_192153: p365 (21) + p366 (51) + p367 (28) + p368 (17) + p369 (25) + p370 (27) + p371 (26) + p372 (36) + p373 (10) + p374 (23) + p377 (20) + p378 (13) + p379 (18) + p380 (24) + p382 (18) + p385 (21) + p388 (19) + p389 (18) + p390 (17) + p392 (20) + p393 (13) + p394 (18) + p396 (19) + p397 (16) + p400 (18) + p401 (27) + p402 (14) = **577/577 ✅**، صفر انحدار.
- **فحوص النظام**: صحة الباك-إند والواجهة 200، Redis PONG، صفر أسطر [ERROR] في آخر 30 دقيقة، صفر مخلفات مستأجرين تجريبيين، وصفر أحداث outbox متبقية من الاختبارات.
- ملاحظة تشغيلية: الأجنحة الأقدم (p365–p382) تطبع ملخص «N passed, M failed» بينما الأحدث تطبع «=== pNNN E2E ===» — مُحلّل الجولة يدعم التنسيقين الآن.

## 2026-09-21 — p402: وسم «ضيف متأخر» للحجوزات المتجاوزة موعدها

**الجديد:** في بطاقة الحجوزات، أي حجز `booked` تجاوز موعده (وقبل وسم `no_show` التلقائي عند ساعتين من p382) يظهر بجانبه وسم أحمر نابض `resv-late-{id}` يعرض دقائق التأخير («⚠ متأخر 25د» / «⚠ Retard 25min») — فيُميّز النادل فورًا بين الحجز الوشيك (عنبري p384) والضيف المتأخر (أحمر) ويتصرف: جلوس أو إلغاء. لا يتداخل مع p382: بعد ساعتين يُوسم تلقائيًا `no_show` فيختفي الوسم.

**الملفات:** `frontend/src/pages/TablesMapPage.js` (واجهة فقط — بلا تغيير خلفية).

**الاختبار:** E2E من 14 نقطة على مستأجر TEST-P402: حجز مستقبلي → نقل موعده 30 دقيقة للماضي → يبقى `booked` (نافذة الوسم) في القائمة وقاعدة البيانات → نقله 3 ساعات للماضي → p382 يوسمه `no_show` مع راية `auto_no_show` (سلامة السلوك القائم) → تحقق وسم `resv-late-` في الحزمة المبنية → تنظيف كامل residue=0. **14/14 ✅**

**النشر:** release `20260921_192153`؛ الصحة 200 ✅.

## 2026-09-21 — p401: تصدير الإقفال اليومي Z كملف CSV

**الجديد:** نقطة `GET /api/restaurant/z-report.csv?date=…` تعيد استخدام `_zreport_payload` حرفيًا (صفر ازدواجية منطق) وتُنتج CSV بترميز UTF-8 مع BOM ليفتحه Excel عربيًا بلا تشويه: ملخص (التاريخ/الطلبات/الملغاة/الخصومات/متوسط الطلب/الإيراد المحصَّل) + أقسام طرق الدفع والأطباق الأكثر طلبًا والتوزيع الساعي. في الواجهة: زر `z-csv-btn` بجانب زر PDF في بطاقة تقرير Z (نفس تاريخ المحدد `zdate`).

**الملفات:** `backend/routes/restaurant_routes.py` + `frontend/src/pages/TablesMapPage.js`.

**الاختبار:** E2E من 27 نقطة على مستأجر TEST-P401: طلبان مدفوعان (كاش 800 + بطاقة 600 مع دلالات الإلحاق بالطلب النشط) → CSV بـ BOM وCRLF وترويسة اسم الملف `z-report-YYYY-MM-DD.csv` → صفوف الملخص وطرق الدفع والأطباق والساعات كلها مطابقة لأرقام JSON → يوم قديم فارغ → تاريخ تالف 400 → بلا مصادقة 401 → تنظيف كامل residue=0. **27/27 ✅**

**النشر:** release `20260921_191131` — تحقق `z-csv-btn` و`z-report.csv` في الحزمة المبنية؛ الصحة 200 ✅.

## 2026-09-21 — p400: بث kitchen_order.created عند تفعيل الطلبيات المجدولة

**المشكلة:** الطلبيات المجدولة (p337) كانت تُفعَّل كسولًا داخل `GET /kitchen-orders` عبر `update_many` أعمى — بلا أحداث SSE، فلا يُنبّه KDS صوتيًا ولا يطبع تلقائيًا (الطباعة التلقائية تحتاج `payload.order_id` من حدث `kitchen_order.created`).

**الحل:** استبدال التحديث الأعمى بحلقة لكل طلب مستحق: تحديث الحالة إلى `pending` ثم `_publish("kitchen_order.created", …)` — فيصل الطلب المفعَّل إلى KDS كأي طلب جديد تمامًا (تنبيه صوتي p381 + طباعة تلقائية).

**الملفات:** `backend/routes/restaurant_routes.py` (خلفية فقط — بلا بناء واجهة).

**الاختبار:** E2E من 18 نقطة على مستأجر TEST-P400: إنشاء مجدول (لا يشغل الطاولة، بلا حدث قبل الأوان) → نقل الموعد للماضي → التفعيل عبر القراءة → `pending` في القائمة وقاعدة البيانات → حدث واحد بالضبط يحمل `order_id` و`status=pending` → بلا تكرار عند إعادة القراءة → اختفاء من لوحة المجدولة → تنظيف كامل (مستأجر + outbox = 0). **18/18 ✅**

**الصحة:** 200 ✅ بعد إعادة التشغيل.

## 2026-09-21 — p399: جولة انحدار شاملة بعد دفعة p396–p398

- **24 جناحًا** على الإصدار المنشور 20260921_183557: p365 (21) + p366 (51) + p367 (28) + p368 (17) + p369 (25) + p370 (27) + p371 (26) + p372 (36) + p373 (10) + p374 (23) + p377 (20) + p378 (13) + p379 (18) + p380 (24) + p382 (18) + p385 (21) + p388 (19) + p389 (18) + p390 (17) + p392 (20) + p393 (13) + p394 (18) + p396 (19) + p397 (16) = **518/518 ✅**، صفر انحدار.
- **فحوص النظام**: صحة الباك-إند والواجهة 200، Redis PONG، صفر أسطر ERROR في آخر 30 دقيقة، صفر مخلفات مستأجرين تجريبيين، وصفر أحداث outbox متبقية من الاختبارات.

## 2026-09-21 — p398: تنبيه صوتي للحجوزات الوشيكة في شاشة الطاولات

- **الواجهة (TablesMapPage)**: تأثير يراقب قائمة الحجوزات — كل حجز booked يدخل نافذة الـ30 دقيقة (نفس عتبة شارة p384) يُنبَّه عنه مرة واحدة: منبثق تحذير 9 ثوانٍ بالاسم/الطاولة/العدد الإضافي + نغمة النادل (محكومة بمفتاح waiter_sound)؛ الحجوزات المُنهاة/الماضية تُنقّى من سجل التنبيه. (التُقط أثناء المراجعة: استيراد useRef كان مفقودًا — أُصلح قبل البناء.)
- **التحقق**: بناء + نشر (release 20260921_183557)؛ السلسلة اللاتينية `resv-soon-alert` ظاهرة في الـ chunk؛ صحة الواجهة والباك-إند 200.

## 2026-09-21 — p397: تصدير أداء الندل CSV

- **الباك-إند**: `GET /api/restaurant/waiter-stats.csv?days=` — يعيد استخدام تجميعة `/waiter-stats`: أعمدة النادل/الطلبات/الملغاة/متوسط الفاتورة/متوسط المدة/الإيراد + صف إجمالي؛ BOM‏ UTF-8 وCRLF واقتباس الخلايا. اسم الملف `waiter-stats-{days}d.csv` مع ضبط الأيام 1–90.
- **الواجهة**: زر `ws-csv-btn` بجانب زر PDF في ترويسة بطاقة «أداء النوادل» — نفس فترة الجدول.
- **E2E (16/16)**: مستأجر TEST-P397 — طلب مدفوع + ملغى: صف النادل (1/1/1600) وصف الإجمالي صحيحان؛ BOM + filename + clamp + رفض بلا توكن + residue=0.

## 2026-09-21 — p396: نقل طلب نشط بين الطاولات

- **الباك-إند**: `PUT /api/restaurant/kitchen-orders/{id}/table` — ينقل طلبًا نشطًا (pending/preparing/pending_payment) إلى طاولة حرة: يحرّر القديمة مع تدوير QR (كما p323)، يشغل الهدف، يحدّث الطلب ويبث kitchen_order.updated. تحققات: 404 للطلب/الطاولة المفقودين، 400 للطلب المنتهي/غير المرتبط/نفس الطاولة/الهدف المشغول.
- **الواجهة**: صف نقل (`order-move-box`) في حوار الطاولة — منتقٍ للطاولات الحرة فقط + زر «نقل» (`order-move-btn`)، يغلق الحوار ويحدّث الخريطة فورًا.
- **E2E (19/19)**: مستأجر TEST-P396 — نقل ناجح (طاولة قديمة free+qr مدوَّر، هدف occupied بالطلب)، 400 لنفس الطاولة/الهدف المشغول/الطلب المنتهي، 404 للمفقودين، رفض بلا توكن، residue=0.

## 2026-09-16 — p395: جولة انحدار شاملة بعد دفعة p392–p394

- **22 جناحًا** على الإصدار المنشور الأخير: p365 (21) + p366 (51) + p367 (28) + p368 (17) + p369 (25) + p370 (27) + p371 (26) + p372 (36) + p373 (10) + p374 (23) + p377 (20) + p378 (13) + p379 (18) + p380 (24) + p382 (18) + p385 (21) + p388 (19) + p389 (18) + p390 (17) + p392 (20) + p393 (13) + p394 (18) = **483/483 ✅**، صفر انحدار.
- **فحوص النظام**: صحة الباك-إند والواجهة 200، Redis PONG، صفر أسطر ERROR في آخر 30 دقيقة، صفر مخلفات مستأجرين تجريبيين، وصفر أحداث outbox متبقية من الاختبارات.
...
- **الواجهة**: زر تحميل (`tstats-pdf-btn`) في ترويسة بطاقة «دوران الطاولات» بجانب منتقي الفترة — بلا تغيير تصميمي آخر.
- **E2E (20/20)**: مستأجر TEST-P392 — طاولة عليها طلبان مدفوعان مُقدَّمان (تقديم الأول يحرّرها للثاني — قاعدة «الطاولة مشغولة» سليمة) وطاولة ميتة بأصفار؛ تطابق JSON؛ PDF: 200 + filename + ‎%PDF + خط مضمّن + صفحة واحدة؛ clamp؛ رفض بلا توكن؛ residue=0.

## 2026-09-16 — p391: جولة انحدار شاملة بعد دفعة اليوم

- **19 جناحًا** على الإصدار المنشور 20260916_144248: p365 (21) + p366 (51) + p367 (28) + p368 (17) + p369 (25) + p370 (27) + p371 (26) + p372 (36) + p373 (10) + p374 (23) + p377 (20) + p378 (13) + p379 (18) + p380 (24) + p382 (18) + p385 (21) + p388 (19) + p389 (18) + p390 (17) = **432/432 ✅**، صفر انحدار (يشمل تأكيد سلامة سلوك p380 الأصلي مع compare=0 بعد تعديل p389).
- **فحوص النظام**: صحة الباك-إند والواجهة 200، Redis PONG، صفر أسطر بمستوى ERROR في آخر 30 دقيقة (التطابقات اللفظية كانت INFO فهارس «0 errors» وإيقاف consume_loop النظيف لإعادة التشغيل)، صفر مخلفات مستأجرين تجريبيين.

## 2026-09-16 — p390: تقرير أداء المطبخ PDF

- **الباك-إند**: `GET /api/restaurant/kitchen-stats.pdf?days=` — تقرير A4 عربي (NotoNaskh) يعيد استخدام تجميعة `/kitchen-stats` باستدعاء داخلي مباشر: ترويسة المتجر، عدد المُقدَّمة، متوسطات الانتظار/التحضير/الإجمالي، عدد المتأخرة ≥15د، أبطأ الأطباق، والتوزيع الساعي المضغوط (توقيت الجزائر). اسم الملف `kitchen-stats-{days}d.pdf` مع ضبط الأيام 1–90.
- **الواجهة (KDS)**: زر تحميل (`kds-stats-pdf`) في شريط إحصاءات المطبخ يحمّل تقرير اليوم (days=1 — نفس فترة الشريط) — بلا تغيير تصميمي آخر.
- **E2E (17/17)**: مستأجر TEST-P390 — طلبان مُقدَّمان بالكامل (preparing→served)؛ JSON count=2؛ PDF: 200 + filename + ‎%PDF + خط مضمّن + صفحة واحدة؛ clamp days=0→7 و500→90؛ رفض بلا توكن؛ residue=0.

## 2026-09-16 — p389: مقارنة الفترة السابقة في التقرير المخصص

- **الباك-إند**: `GET /api/restaurant/period-report?compare=1` — معامل اختياري إضافي بالكامل (compare=0 يحافظ على سلوك p380 حرفيًا بلا مفتاحي prev/delta): يضيف `prev` (طلبات/ملغاة/إيراد/خصومات الفترة السابقة المساوية في الطول مباشرة مع حدودها) و`delta` (نسبة التغير في الإيراد والطلبات؛ ‏100% عند فترة سابقة فارغة مع حالية > 0، وnull عند فراغهما معًا).
- **الواجهة**: زر تبديل (`period-compare-toggle`، أيقونة %) بجانب «عرض» — يعيد الجلب فورًا إن كان التقرير معروضًا؛ سطر مقارنة (`period-compare`) يعرض دلتا الإيراد (`period-delta-revenue`) والطلبات (`period-delta-orders`) بالأخضر/الأحمر مع حدود الفترة السابقة.
- **E2E (18/18)**: مستأجر TEST-P389 — طلبان حاليان (2400) وطلب سابق (1000): compare=0 بلا prev/delta؛ compare=1 حدود سابقة صحيحة + delta إيراد ‎+140%‎ وطلبات ‎+100%‎؛ سابقة فارغة → 100؛ فارغتان → null؛ رفض بلا توكن؛ residue=0.

## 2026-09-16 — p388: تقرير الحجوزات PDF لفترة

- **الباك-إند**: `GET /api/restaurant/reservations/report.pdf?date_from&date_to` — تقرير A4 عربي (NotoNaskh) يعيد استخدام `_period_bounds` (افتراضي: اليوم−6 → اليوم، 400 للمدى المعكوس/الفاسد/أكثر من 92 يومًا): ترويسة المتجر، عدّادات الحالات (مؤكد/حضر/ملغى/لم يحضر)، نسبة عدم الحضور، إجمالي الضيوف، ثم قائمة تفصيلية مرتبة زمنيًا (توقيت الجزائر) لكل حجز بحالته. اسم الملف `reservations-report-{from}_{to}.pdf`.
- **الواجهة**: زر تحميل (`resv-pdf-btn`) بجانب «حجز جديد» في بطاقة الحجوزات — بلا تغيير تصميمي آخر.
- **E2E (19/19)**: مستأجر TEST-P388 — 4 حجوزات بكل الحالات (بما فيها no_show التلقائي p382)؛ PDF: 200 + filename بالفترة الافتراضية والصريحة + ‎%PDF + خط مضمّن + صفحة واحدة؛ 400 للمدى المعكوس و>92 يومًا؛ رفض بلا توكن؛ residue=0.

## 2026-09-13 — p387: جولة الانحدار الختامية الشاملة

- **16 جناحًا** على الإصدار المنشور 20260913_185427: p365 (21) + p366 (51) + p367 (28) + p368 (17) + p369 (25) + p370 (27) + p371 (26) + p372 (36) + p373 (10) + p374 (23) + p377 (20) + p378 (13) + p379 (18) + p380 (24) + p382 (18) + p385 (21) = **378/378 ✅**، صفر انحدار. (فشل عابر واحد في p366/residue-db بسبب سبق توقيت 2 ثانية لحذف القاعدة — نجح عند الإعادة 51/51.)
- **فحوص النظام**: صحة الباك-إند والواجهة 200، Redis PONG، صفر أسطر ERROR في آخر 30 دقيقة، صفر ضجيج consume_loop، صفر مخلفات مستأجرين تجريبيين (TEST-P3*).

## 2026-09-13 — p386: تصعيد صوتي للطلبات المتأخرة في شاشة المطبخ

- **الواجهة (KitchenDisplayPage)**: تأثير يراقب قائمة الطلبات — كل طلب pending/preparing يتجاوز 15 دقيقة (نفس عتبة العلامة البصرية القائمة) يُنبَّه عنه مرة واحدة بنغمة تصعيد ثلاثية تصاعدية (660→880→1320Hz) مميِّزة عن نغمة الطلب الجديد + منبثق تحذير 9 ثوانٍ برمز الطلب وعدد الإضافيين؛ الطلبات المُنهاة/الملغاة تُنقّى من سجل التنبيه، وكل ذلك محكوم بمفتاح صوت المطبخ القائم (kds_sound).
- **التحقق**: بناء + نشر؛ السلسلة اللاتينية `kds-late-escalation` (console.info) ظاهرة في الـ chunk المبني؛ JSX مُتحقق منه؛ صحة الواجهة والباك-إند 200.

## 2026-09-13 — p385: تقرير أداء الندل PDF

- **الباك-إند**: `GET /api/restaurant/waiter-stats.pdf?days=` — تقرير A4 عربي (خط NotoNaskh مضمّن) يعيد استخدام تجميعة `/waiter-stats` نفسها باستدعاء داخلي مباشر (لا تغيير على مسار JSON): ترويسة المتجر، الفترة، إجمالي الندل/الطلبات/الإيراد، ثم الندل مرتّبين حسب الإيراد مع طلبات/ملغاة/متوسط فاتورة/متوسط مدة لكل نادل. اسم الملف `waiter-stats-{days}d.pdf` مع ضبط الأيام 1–90.
- **الواجهة**: زر تحميل (`ws-pdf-btn`) في ترويسة بطاقة «أداء النوادل» يحمّل PDF لنفس فترة الجدول (tstatsDays) — بلا أي تغيير تصميمي آخر.
- **E2E (21/21)**: مستأجر TEST-P385 — طلب مدفوع كامل + جزئي (item_indexes) + ملغى؛ تطابق JSON (orders/cancelled/revenue=1900/avg_bill=1200)؛ PDF: 200 + content-type + filename + ‎%PDF + خط مضمّن + صفحة واحدة؛ clamp days=0→7 و500→90؛ رفض بلا توكن؛ تنظيف cascade وresidue=0.

## 2026-09-13 — p384: تنبيه الحجوزات الوشيكة في بطاقة الحجوزات

- **الواجهة (TablesMapPage)**: شارة كهرمانية نابضة «⏰ قادم قريبًا» (`resv-soon-{id}`) على كل حجز بحالة booked يحين موعده خلال 30 دقيقة، وعدّاد إجمالي الحجوزات المؤكدة (`resv-booked-count`) بجانب عنوان بطاقة «حجوزات الطاولات» — دون أي تغيير في التصميم أو الألوان القائمة.
- **التحقق**: بناء + نشر (release 20260913_182217)؛ testidان ظاهران في الـ chunk المبني؛ صحة الواجهة 200 ونقاط الحجوزات تستجيب (401 دون توكن = المسار موجود ومحمي).

## 2026-09-13 — p383: جولة انحدار ثانية بعد مراحل اليوم
- **لا تغيير برمجي** — إعادة تشغيل 9 أجنحة E2E ضد النشرة الحالية (`20260913_150956`)، تشمل كل ما لمسته إعادات البناء اليوم (نواة `_zreport_agg`، مساعد `_receipt_payload`، قائمة الحجوزات بعد p382):

  | الجناح | النتيجة | | الجناح | النتيجة | | الجناح | النتيجة |
  |---|---|---|---|---|---|---|---|
  | p370 الحجوزات | 27/27 | | p374 المنحنى | 23/23 | | p379 تقرير Z PDF | 18/18 |
  | p371 تقرير Z | 26/26 | | p377 إيصال PDF | 20/20 | | p380 تقرير الفترة | 24/24 |
  | p372 إيصال الزبون | 36/36 | | p378 تنبيه النادل | 13/13 | | p382 no_show التلقائي | 18/18 |

  **الإجمالي: 205/205 ✅ — صفر انحدارات.** (مع جولة p376: 469 اختبارًا أخضر إجمالًا على مدار اليوم.)
- **فحوص النظام**: health=200، بقايا حسابات الاختبار في `identity_registry` = 0، آخر التزامات p377–p382 مدفوعة كلها إلى GitHub.

## 2026-09-13 — p382: تأشير الحجوزات الفائتة تلقائيًا (no_show كسول)
- **الخلفية**: `routes/restaurant_routes.py` — في `GET /restaurant/reservations`، قبل الجلب تُوسم كسولًا كل حجوزات `booked` التي مضى على موعدها **أكثر من ساعتين** بلا حضور: `status=no_show` + `auto_no_show=true` + ختم تحديث. النتيجة: فترة سماح ساعتان للتأخر، تحرير تلقائي للطاولة من الحجوزات الميتة (تعارض p370 يحتسب `booked` فقط)، وبلا مجدول خلفية إضافي. الحالة النهائية تظل نهائية: إعادة الجلوس على no_show تلقائي = 400.
- **اختبارات**: `p382_e2e.py` — **18/18**: رفض الحجز في الماضي (p370 سليم)، محاكاة مرور الوقت عبر نقل `reserved_for` في القاعدة مباشرة (A←قبل 3.5س، B←قبل ساعة)، عند القائمة: A تُوسم no_show تلقائيًا (يُفحص من القاعدة لأنه خارج نافذة القائمة now−3h)، B ضمن السماح يبقى booked، C المستقبلي booked، إنهائية no_show (جلوس = 400)، مسارا seated/cancelled العاديان سليمان، ثبات عند إعادة القائمة، حجز جديد طبيعي بعدها، وتنظيف كامل residue=0.

## 2026-09-13 — p381: تنبيه صوتي في شاشة المطبخ عند وصول طلب جديد
- **الواجهة فقط**: `KitchenDisplayPage.js` — عند حدث SSE `kitchen_order.created` (الذي يغذي الطبع التلقائي p338 أصلًا) تُعزف **نغمة WebAudio ثنائية النبرة** (880→1320Hz، بلا ملفات) ويظهر منبثق قصير بكود الطلب والطاولة. زر تبديل جديد (`kds-sound-toggle`) بجانب زر الطبع التلقائي، محفوظ في `localStorage` (مفعّل افتراضيًا). لا تغيير على الخلفية — حمولة الحدث أثبتتها p378 بالفعل.
- **تحقق**: تحليل JSX (babel) سليم، البناء والنشر ناجحان، testids اللاتينية (`kds-sound-toggle`, `kds_sound`) في الحزمة المبنية.

## 2026-09-13 — p380: تقرير فترة مخصصة (من/إلى) للمطعم — JSON وPDF
- **الخلفية**: `routes/restaurant_routes.py` — استُخرجت نواة تجميع Z-report إلى `_zreport_agg(start, end)` (مشتركة بين اليوم الواحد والفترات بلا ازدواجية)، وأُضيفا `GET /restaurant/period-report?date_from=&date_to=` و`period-report.pdf`: الافتراضي آخر 7 أيام، تحقق 400 للتاريخ الفاسد/المعكوس/الفترة > 92 يومًا، حدود الأيام بتوقيت الجزائر، ونفس دلالات p371 (بلا ملغاة، إيراد يستثني الآجل). نتيجة اليوم الواحد عبر المسار الجديد **مطابقة تمامًا** لـ z-report (مُثبت بالاختبار). الـ PDF بنمط p379: A4، خط عربي مضمَّن، عنوان «تقرير الفترة» مع المدى، اسم الملف `period-report-{from}_{to}.pdf`.
- **الواجهة**: `TablesMapPage.js` — بطاقة «تقرير فترة مخصصة» (`period-card`): منتقيا تاريخ + زر «عرض» (يُجلب عند الضغط فقط) + زر PDF معطَّل بلا بيانات، وأربع خلاصات (أيام/طلبات/إيراد/خصومات) وشارات الطرق وأعلى 5 أطباق.
- **اختبارات**: `p380_e2e.py` — **24/24**: المدى الافتراضي (7 أيام)، القيم (طلبتان، ملغى، إيراد 400 بلا آجل 200)، تطابق اليوم الواحد مع z-report حقلًا بحقل (methods/dishes)، فترة فارغة بأصفار، 400×3 (فاسد/معكوس/>92)، 401، PDF (magic/content-type/اسم الملف/الخط المضمَّن)، سلامة z-report JSON وPDF بعد إعادة البناء، وتنظيف كامل residue=0.

## 2026-09-13 — p379: تقرير الإقفال Z كملف PDF قابل للتنزيل والأرشفة
- **الخلفية**: `routes/restaurant_routes.py` — استُخرج منطق Z-report إلى مساعد `_zreport_payload` (يستخدمه مسارا JSON p371 وPDF الجديد بلا ازدواجية)، وأُضيف `GET /restaurant/z-report.pdf?date=YYYY-MM-DD`: تقرير A4 عمودي عبر reportlab + Noto Naskh Arabic مضمَّن (تشكيل + bidi، نمط p377): اسم المتجر، التاريخ، الطلبات/الملغاة/الخصومات/متوسط الطلب، التحصيل حسب الطريقة مع الأعداد، الإيراد المحصَّل (بلا الآجل) بخط أكبر، الأكثر طلبًا، والتوزيع الساعي مكثَّفًا، مع فواصل متقطعة وتجاوز تلقائي للصفحات. تاريخ فاسد = 400.
- **الواجهة**: `TablesMapPage.js` — زر تنزيل PDF (`z-pdf-btn`) بجانب زر الطباعة في ترويسة بطاقة Z-report؛ يحترم منتقي التاريخ الحالي ويُنزِّل باسم `z-report-{date}.pdf`.
- **اختبارات**: `p379_e2e.py` — **18/18**: JSON سليم بعد إعادة البناء (طلبات 2، ملغى 1، إيراد 400 بلا الآجل)، PDF حقيقي (`%PDF-`، content-type، اسم الملف بتاريخ اليوم، حجم ~15KB، خط مضمَّن، صفحة واحدة عبر pypdf)، معامل التاريخ الصريح، تاريخ فاسد 400، 401، وتنظيف كامل residue=0.

## 2026-09-13 — p378: تنبيه النادل عند جاهزية الطلب (صوتي + منبثق)
- **الواجهة**: `TablesMapPage.js` — اشتراك SSE مخصص في `kitchen_order.updated`: عند وصول `status === 'served'` (زر «جاهز» في شاشة المطبخ) يظهر تنبيه منبثق بارز (8 ثوانٍ) بكود الطلب واسم الطاولة + **نغمة WebAudio** (مذبذب 880Hz بلا أي ملفات صوتية). زر تبديل في ترويسة الصفحة (`waiter-sound-toggle`، أيقونة Volume2/VolumeX) يفعّل/يكتم الصوت، محفوظ في `localStorage` (مفعّل افتراضيًا). لا اعتماد على الجلب الدوري — التنبيه لحظي عبر SSE.
- **اختبارات**: `p378_e2e.py` — **13/13**: تدفق الحالات pending→preparing→served، والحدث `kitchen_order.updated` يصل صندوق الإرسال (outbox) بالحمولة الدقيقة التي يستهلكها معالج الواجهة (`status=served`، `table_name`، `code=KCH-…`، `order_id`)، وتنظيف كامل residue=0. تحليل JSX سليم وtestids اللاتينية في الحزمة المبنية.

## 2026-09-13 — p377: إيصال الزبون PDF قابل للتنزيل والمشاركة
- **الخلفية**: `routes/restaurant_routes.py` — استُخرجت بيانات الإيصال إلى مساعد `_receipt_payload` (يستخدمه مسارا JSON p372 وPDF الجديد بلا ازدواجية)، وأُضيف `GET /restaurant/kitchen-orders/{id}/receipt.pdf`: إيصال بعرض طابعة 80مم عبر reportlab + خط **Noto Naskh Arabic** المضمَّن مع التشكيل وbidi (نفس نمط import_export المجرّب): ترويسة المتجر والكود والطاولة والوقت، العناصر، المجموع/الخصم/الصافي، تفصيل دفعات p366 بأرقام الفواتير، المدفوع/المتبقي، وعبارة شكر. يعمل للطلبات غير المدفوعة (فاتورة أولية) والمدفوعة.
- **الواجهة**: `TablesMapPage.js` — زر «تحميل الفاتورة PDF» (`receipt-pdf-btn`) بجانب زر الطباعة في حوار الطاولة؛ جلب مصادق عليه بصيغة blob ثم تنزيل باسم `receipt-{code}.pdf`.
- **اختبارات**: `p377_e2e.py` — **20/20**: PDF حقيقي (`%PDF-` magic، content-type، اسم الملف يحمل كود الطلب، حجم >12KB، خط عربي مضمَّن، صفحة واحدة عبر pypdf)، قبل الدفع وبعده (خصم 50 → صافي 750)، مسار JSON p372 سليم بعد إعادة البناء، 404/401، وتنظيف كامل residue=0.

## 2026-09-12 — p376: جولة انحدار شاملة على النظام المنشور
- **لا تغيير برمجي** — إعادة تشغيل أجنحة E2E العشرة الأخيرة كاملة ضد النشرة الحالية (`20260912_020419`):

  | الجناح | النتيجة | | الجناح | النتيجة |
  |---|---|---|---|---|
  | p365 وسوم الحمية | 21/21 | | p371 تقرير Z | 26/26 |
  | p366 تقسيم الفاتورة | 51/51 | | p372 فاتورة الزبون | 36/36 |
  | p367 دوران الطاولات | 28/28 | | p373 ناقل الأحداث | 10/10 |
  | p368 تبديل اللغة | 17/17 | | p374 منحنى الإيرادات | 23/23 |
  | p369 أداء الندل | 25/25 | | p370 الحجوزات | 27/27 |

  **الإجمالي: 264/264 ✅ — صفر انحدارات.**
- **فحوص النظام**: health=200، Redis PONG سليم، صفر أسطر أخطاء في سجلات آخر 30 دقيقة، صفر ضجيج مهلات `consume_loop` منذ p373، وبقايا حسابات الاختبار في `identity_registry` = 0 (كل المستأجرين التجريبيين TEST-P3xx حُذفوا بالتتالي).

## 2026-09-12 — p375: طباعة تقرير إقفال اليوم (Z)
- **الواجهة فقط**: `TablesMapPage.js` — زر «طباعة» (`z-print-btn`) في ترويسة بطاقة Z-report (معطَّل عند غياب بيانات اليوم) يطبع منطقة مخفية `#z-print` بنمط تذكرة KDS نفسه: التاريخ، الطلبات/الملغاة/الخصومات/متوسط الطلب، التحصيل حسب الطريقة مع الأعداد، الإيراد المحصَّل (بلا الآجل) بخط عريض، وأكثر 8 أطباق. أُضيفت حالة `printMode` (receipt|z) لتفادي ظهور فاتورة الزبون (p372) وتقرير الإقفال معًا في ورقة طباعة واحدة. لا تغيير على شكل الديسكتوب — كل المناطق المضافة مخفية خارج الطبع.
- **تحقق**: تحليل JSX (babel) سليم، البناء والنشر ناجحان، testids اللاتينية (`z-print-btn`, `z-print`, `z-print-date`, `z-print-revenue`) موجودة في الحزمة المبنية؛ البيانات المعروضة مغطاة أصلًا باختبارات p371 (26/26).

## 2026-09-12 — p374: منحنى إيرادات المطعم متعدد الأيام
- **الخلفية**: `routes/restaurant_routes.py` — `GET /restaurant/revenue-trend?days=N` (1–90، الافتراضي 7 و`days=0`→7 كاتفاق p363): تجميع يومي **بتوقيت الجزائر** (`$dateToString` Africa/Algiers) يعيد سلسلة `{date, orders, revenue, discounts}` — الطلبات تستثني الملغاة، و`revenue` = المحصَّل النقدي الفعلي **يستثني الآجل** عبر توحيد دفعات p366 (`payments[]`) مع الطلبات الأقدم المدفوعة (نفس دلالة z-report)، مع ملء الأيام الفارغة بأصفار، ومجاميع `total_orders`/`total_revenue`.
- **الواجهة**: `TablesMapPage.js` — بطاقة «منحنى الإيرادات المحصَّلة» (`trend-card`) بمنتقي فترة (7/14/30) وأعمدة CSS بسيطة (ارتفاع نسبي للأعلى، قيمة وتاريخ كل يوم، عنوان tooltip بالتفاصيل) + إجمالي الفترة في العنوان. تُجلب عند تغيير الفترة فقط.
- **اختبارات**: `p374_e2e.py` — **23/23**: طول السلسلة 7، صف اليوم (3 طلبات، إيراد 700 = كاش 300 + بطاقة 400 بعد خصم 100، الآجل 200 مستثنى، الملغي مستثنى)، خصومات 100، المجاميع، فجوة الأمس بأصفار، days=1/0(→7)/200(→90)/abc(→422)، 401، وتنظيف كامل residue=0.

## 2026-09-12 — p373: تهدئة ضجيج مهلة Redis في سجلات ناقل الأحداث
- **الخلفية**: `services/event_bus.py` — حلقة `consume_loop` كانت تسجّل مهلة القراءة الحجبية لـ `XREADGROUP` (حميدة، تُعاد المحاولة تلقائيًا) كخطأ ERROR مع traceback متسلسل كامل (~مرتين/يوم، يضخّم عدّ الأخطاء). الآن: استثناءات المهلة (`Timeout` في الاسم/النص) تُسجَّل سطر debug واحدًا بلا traceback، وكل الأخطاء الأخرى تبقى `log.exception` كاملة كما كانت.
- **اختبارات**: `p373_e2e.py` — **10/10**: سلامة الواجهة بعد إعادة التشغيل، حدث `kitchen_order.created` يُستهلك فعليًا (`processed_events` ازداد 19→20)، صفر أسطر `consume_loop iteration failed` منذ إعادة التشغيل، المستهلك ما زال يعمل، وتنظيف كامل residue=0.

## 2026-09-11 — p372: فاتورة/إيصال الزبون للمطعم (طباعة)
- **الخلفية**: `routes/restaurant_routes.py` — `GET /restaurant/kitchen-orders/{id}/receipt` (مصادقة؛ 404 لغير الموجود): يعيد بيانات جاهزة للطباعة — اسم المتجر (من `saas_tenants`، بلا بوابة ميزة)، كود الطلب، الطاولة، الحالة وحالة الدفع، الموظف، طوابع الإنشاء/التقديم، العناصر (كمية/سعر/إجمالي/مؤشر مدفوع)، المجموع/الخصم/الصافي عبر `_order_out`، **تفصيل دفعات p366** (طريقة/مبلغ/دافع/وقت/رقم فاتورة البيع)، والمدفوع/المتبقي مع استرجاع الطلبات القديمة المدفوعة بلا سجل دفعات (`paid_amount = final_total`).
- **الواجهة**: `TablesMapPage.js` — زر «طباعة فاتورة الزبون» (`receipt-print-btn`) في حوار الطاولة للطلب النشط (متاح قبل الدفع كفاتورة أولية وبعده كإيصال) + منطقة طباعة مخفية `#receipt-print` بنمط تذكرة KDS نفسه (`@media print` + `visibility`): ترويسة المتجر والكود والطاولة والوقت، جدول العناصر (الغير مدفوع يُوسم)، المجموع/الخصم/الصافي، سطر لكل دفعة مع رقم فاتورتها، المدفوع/المتبقي، وعبارة شكر. لا تغيير على شكل الديسكتوب — المنطقة مخفية تمامًا خارج الطبع.
- **اختبارات**: `p372_e2e.py` — **36/36**: ترويسة/كود/طاولة/عناصر غير مدفوع (مدفوع 0، متبقٍ = الصافي)، 404/401، بعد خصم 100 ودفعة بطاقة جزئية (partial، دفعة واحدة 533.33 مع رقم فاتورة INV-، مدفوع/متبقٍ 533.33/266.67، مؤشرات العناصر)، بعد إتمام الدفع نقدًا (دفعتان، مدفوع = 800 بالضبط، متبقٍ 0، كل العناصر مدفوعة)، وطلب قديم مدفوع بلا `payments[]` (استرجاع `paid_amount = final_total`)، وتنظيف كامل residue=0.

## 2026-09-10 — p371: تقرير إقفال اليوم (Z-report) للمطعم
- **الخلفية**: backend (`routes/restaurant_routes.py`) — `GET /restaurant/z-report?date=YYYY-MM-DD` (الافتراضي اليوم **بتوقيت الجزائر**؛ تاريخ فاسد = 400): `$facet` على طلبات اليوم يعيد الطلبات (بلا الملغاة) والملغاة ومجموع الخصومات ومتوسط الطلب، وتفصيل التحصيل حسب الطريقة (كاش/بطاقة/آجل) عبر توحيد دفعات p366 (`payments[]`) مع الطلبات الأقدم المدفوعة (تُشتق من `payment_method`)، و`revenue` = المحصَّل النقدي الفعلي **يستثني الآجل**، وأكثر 8 أطباق طلباً وتوزيع ساعي (Africa/Algiers).
- **الواجهة**: `TablesMapPage.js` — بطاقة «تقرير إقفال اليوم (Z)» مع منتقي تاريخ: خمس خلاصات رقمية (طلبات/ملغاة/إيراد/خصومات/متوسط)، شارات التحصيل حسب الطريقة، وأكثر 5 أطباق. تُجلب عند تغيير التاريخ فقط.
- **الاختبارات**: `p371_e2e.py` = 26/26 E2E_GREEN — كاش 500 + بطاقة 900 (بعد خصم 100) + آجل 500: التفصيل صحيح والإيراد 1400 يستثني الآجل؛ الملغي خارج الطلبات والخصومات؛ متوسط الطلب 633.33؛ أعلى طبق ×4؛ يوم أمس = أصفار؛ تاريخ فاسد 400؛ بلا مصادقة 403؛ حذف المستأجر وresidue=0.

## 2026-09-08 — p370: حجوزات الطاولات المسبقة
- **الخلفية**: backend (`routes/restaurant_routes.py`) — مجموعة `restaurant_reservations` مع `GET /restaurant/reservations?days=` (1–30، من الآن−3س)، و`POST /restaurant/reservations` (طاولة موجودة، اسم إلزامي، موعد ISO مستقبلي — بلا منطقة يُفترض توقيت الجزائر، 1–50 شخصاً)، وكشف تعارض ±90 دقيقة **بحدّين حصريين** (فرق 90د بالضبط مسموح) يُعيد 409، و`PUT /reservations/{id}/status` (booked→seated/cancelled/no_show؛ المنتهي cancelled/no_show محمي 400). إلغاء الحجز يحرر نافذته فوراً.
- **الواجهة**: `TablesMapPage.js` — بطاقة «حجوزات الطاولات» في الجلب الدوري (15ث): قائمة قادمة مرتبة زمنياً (اسم/طاولة/أشخاص/موعد) مع زرّي «جلوس» و«إلغاء»، وشارات الحالة؛ نافذة «حجز جديد» (اختيار طاولة، اسم، هاتف، datetime-local، عدد أشخاص).
- **الاختبارات**: `p370_e2e.py` = 27/27 E2E_GREEN — إنشاء وتطبيع الهاتف (0555→213555)؛ تعارض 409 وطاولة أخرى لا تتعارض؛ ماضٍ/طاولة مفقودة/اسم فارغ = 400/404/400؛ قائمة مرتبة تصاعدياً ونافذة الأيام؛ seated/cancelled/terminal-400؛ تحرير النافذة بعد الإلغاء؛ بلا مصادقة 403؛ حذف المستأجر وresidue=0.

## 2026-09-08 — p369: تحليلات أداء النوادل
- **الخلفية**: backend (`routes/restaurant_routes.py`) — `GET /restaurant/waiter-stats?days=` (1–90، الافتراضي 7): تجميع `$group` على `created_by` لطلبات المطبخ **يستثني قنوات الزبون الذاتية** (`QR`/`KIOSK`/`جار:*`) ويعيد لكل موظف: الطلبات (بلا الملغاة)، عدد الملغاة، الإيراد المحصَّل (كامل الفاتورة إن `paid` وإلا مجموع دفعات p366)، متوسط الفاتورة، ومتوسط مدة الجلسة بالدقائق (ختوم p363). الترتيب تنازلي حسب الإيراد.
- **الواجهة**: `TablesMapPage.js` — بطاقة «أداء النوادل» تحت بطاقة دوران الطاولات (p367) وتشاركها محدد الفترة: جدول الموظف/الطلبات/الملغاة (بأحمر)/متوسط الفاتورة/مدة الجلسة/الإيراد. تُجلب مع تغيير الفترة فقط.
- **ملاحظة سلوكية موثقة**: إنشاء طلب على طاولة عليها طلب نشط يُلحق العناصر بالطلب القائم (سلوك قديم مقصود) — كشفه تشخيص p369 أثناء الاختبار.
- **الاختبارات**: `p369_e2e.py` = 25/25 E2E_GREEN — طلب كامل مدفوع + دفعة جزئية (p366) تُحسبان معاً (revenue=1000، avg_bill=750)؛ الملغي يُحسب في cancelled ولا يدخل الطلبات/الإيراد؛ طلب QR على طاولة ثانية مُستبعَد كلياً؛ كلاَمب 90؛ بلا مصادقة 403؛ حذف المستأجر وresidue=0.

## 2026-09-08 — p368: مفتاح اللغة عربي/إنجليزي في القوائم العامة
- **الخلفية**: backend (`routes/restaurant_routes.py`) — كتل تخطيط القوائم العامة الثلاث (public menu/table-menu/kiosk) تعرض الآن `name_en` ضمن كل عنصر (كان الحقل يُجلب من مونجو ولا يُرسل للواجهة).
- **الواجهة**: `QrMenuPage.js` و`KioskPage.js` — زر تبديل اللغة (EN/عربي) في الترويسة يُحفظ في `localStorage` (`pub_lang`) ويشمل: أسماء الأطباق (`name_en` مع سقوط للاسم العربي)، قاموس نصوص كامل للأزرار والعناوين والسلة وشاشتي النجاح والعداد التنازلي وعناصر الإدخال، أسماء وسوم الحساسية (`TAG_EN`)، العملة (دج/DA)، وقلب الاتجاه `dir` تلقائياً (RTL/LTR) في كل الشاشات بما فيها الخطأ والتحميل. عناصر السلة تحمل `name_en` فتتبع تبديل اللغة حتى بعد الإضافة.
- **الاختبارات**: `p368_e2e.py` = 17/17 E2E_GREEN — `name_en` يظهر في القوائم العامة الثلاث (مع 404 عند تعطيل الكشك)؛ التحقق من الواجهة عبر testid ‏`lang-toggle` وسلسلة `pub_lang` في الحزم المبنية؛ رجعي p365 = 21/21؛ حذف المستأجر وresidue=0.

## 2026-09-08 — p367: تحليلات دوران الطاولات
- **الخلفية**: backend (`routes/restaurant_routes.py`) — `GET /restaurant/table-stats?days=` (1–90، 0/فارغ = 7 كما p363): تجميع `$group` على طلبات المطبخ المربوطة بطاولة (يستثني الملغاة والمجدولة) يعيد لكل طاولة: عدد الطلبات، الإيراد المحصَّل (الفاتورة كاملة إن `paid`، وإلا مجموع دفعات `payments` — يحسب الدفعات الجزئية من p366 بدقة)، متوسط الفاتورة، متوسط مدة الجلسة بالدقائق (من ختوم p363 الزمنية)، وطلب/يوم. الطاولات بلا طلبات تظهر بأصفار لكشف الطاولات الميتة، والترتيب تنازلي حسب الإيراد مع إجماليات.
- **الواجهة**: `TablesMapPage.js` — بطاقة «دوران الطاولات» (محدد فترة 7/30/90 يوماً، إجمالي الطلبات والإيراد، جدول لكل طاولة: طلبات، طلب/يوم، متوسط الفاتورة، مدة الجلسة، الإيراد)؛ تُجلب عند الفتح وتغيير الفترة فقط — لا تدخل الجلب الدوري كل 15 ثانية.
- **الاختبارات**: `p367_e2e.py` = 28/28 E2E_GREEN — طاولتان عاملتان + ميتة؛ مدفوع كامل كاش/بطاقة + دفعة جزئية (p366) تُحسب 500 من 800؛ الملغي مُستبعَد؛ الإجماليات والترتيب؛ كلاَمب 90 وdays=0→7؛ بلا مصادقة 403؛ حذف المستأجر وresidue=0.

## 2026-09-08 — p366: تقسيم فاتورة طلب المطعم (Split Bill)
- **الخلفية**: backend (`routes/restaurant_routes.py`) — `PayBody` يقبل الآن `item_indexes` (فهارس العناصر المدفوعة في الدفعة؛ `None` = كل غير المدفوع = السلوك السابق حرفياً). كل دفعة نقدية/بطاقة تُنشئ فاتورة بيع حقيقية خاصة بها (امتداد p362: مخزون + صندوق + محاسبة + ولاء) وتُسجَّل في `payments[]` و`sale_ids[]` على الطلب (أول فاتورة تبقى في `sale_id` توافقاً). العناصر المدفوعة تُعلَّم `paid`، وحالة الدفع الجديدة `partial` حتى اكتمال السداد. الخصم (p337) يُوزَّع تناسبياً على الدفعات والدفعة الأخيرة تسدد الباقي بالضبط فيطابق المجموع `final_total` دائماً. النمط المسبق (prepaid) يرفض الدفع الجزئي (400) — لا يدخل المطبخ إلا مكتمل السداد. الآجل debt يعلّم العناصر مدفوعة بلا قيد بيع (كما p362). إعادة دفع فاتورة مسددة لا تغيّر شيئاً (idempotent)، ودفع عنصر مدفوع = 400.
- **`_order_out`** يعرض الآن `paid_amount` و`remaining_total` و`unpaid_count` لكل طلب.
- **الواجهة**: `TablesMapPage.js` — قسم «تقسيم الفاتورة» داخل نافذة الطاولة: اختيار العناصر غير المدفوعة (المدفوعة تظهر مشطوبة)، طريقة الدفع (كاش/بطاقة)، شريط تقدم (المدفوع/المتبقي)، وشارة «مدفوع جزئياً» كهرمانية على النافذة وبطاقة الطاولة؛ `KitchenDisplayPage.js` — شارة «مدفوع جزئياً» على بطاقة الطلب.
- **الاختبارات**: `p366_e2e.py` = 51/51 E2E_GREEN — دفع كامل توافقي + idempotency؛ دفعتان (كاش ثم بطاقة) بفاتورتين مجموعهما = الإجمالي والمخزون ينقص بدقة؛ خصم 200 على 1800 → دفعات 444.44/266.67/888.89 مجموعها 1600 بالضبط وكل فاتورة (subtotal−discount=total)؛ المسبق يرفض الجزئي ويقبل الكامل؛ debt جزئي بلا فاتورة؛ فهارس خاطئة/فارغة/مدفوعة = 400؛ حذف المستأجر وresidue=0. رجعي: p362 = 27/27 وp360 = 28/28 بعد إعادة البناء.

## 2026-09-08 — p365: علامات الحساسية والنظام الغذائي للأطباق
- **الخلفية**: backend (`routes/restaurant_routes.py`) — ثابت `DIETARY_TAGS` (gluten/nuts/dairy/egg/fish/sesame/spicy/vegan/vegetarian)؛ `GET /restaurant/products/{id}/dietary` + `PUT` (مالك، تحقق من القائمة البيضاء + dedupe، 400 لوسم غير معروف)؛ القوائم العامة الثلاث (public menu/table-menu/kiosk) تعرض الآن `dietary_tags` ضمن كل عنصر — قناة p316 تُعرِّف الوسوم وصفحة الإنتاج تديرها.
- **الواجهة**: `ProductionPage.js` زر «العلامات» لكل وصفة مع نافذة اختيار (9 وسوم بأسماء عربية)؛ `QrMenuPage.js` و`KioskPage.js` شارات صغيرة بجانب العناصر الموسومة.
- **الاختبارات**: `p365_e2e.py` = E2E_GREEN — إنشاء مستأجر TEST-P365؛ ضبط/قراءة/تدقيق الأوسمة (غير معروف 400، dedupe، منتج غير موجود 404)؛ ظهورها في QR + الكشك (مع 404 عند التعطيل) + table-menu (410 بعد تدوير التوكن ما زال)؛ مسح الأوسمة ينعكس عاماً؛ حذف المستأجر وresidue=0.

## 2026-09-08 — p364: من التوقع إلى الشراء بنقرة — مسودة شراء آلية من تنبؤ الطلب

**الفكرة**: تنبؤ p313 كان يخبرك «الطحين يكفي يوماً ونصف — اشترِ 24» ثم يتركك تدخل الكميات يدوياً في فاتورة شراء. الآن زر واحد يحوّل الاقتراح إلى مسودة جاهزة للمراجعة.
- نقطة `POST /production/demand-forecast/create-purchase {supplier_id, days, cover, product_ids?}`: تعيد استخدام منطق demand_forecast نفسه، تنتقي الأصناف المقترحة (أو المحددة)، وتنشئ فاتورة شراء **مسودة** (`stock_status: "draft"`, `source: "forecast"`, غير مدفوعة) برقم PUR- اليومي الذرّي نفسه — الكمية = سقف المقترح (PurchaseItem.quantity صحيح)، السعر = purchase_price الحالي
- المسودة لا تلامس المخزون ولا الأسعار؛ عند تأكيدها من مسار المشتريات القائم يسري كل شيء تلقائياً: مزامنة purchase_price (p171) وتنبيه انحراف الكلفة (p359)
- واجهة: في تبويب «توقع الطلب» — منتقي مورد + زر «إنشاء مسودة شراء» يظهر فقط عند وجود اقتراحات، وtoast برقم الفاتورة
- اختبار 22/22 على TEST-P364: استهلاك 20 وحدة بيعاً (المخزون ينقص بالوصفة إلى −19 كما صمّم p303) ← suggested 23.667 وdays_remaining −28.5 وurgent، مورد وهمي 404، بلا انتقاء 400، المسودة (24 وحدة × 100 = 2400، draft، غير مدفوعة)، المخزون لا يتغير قبل التأكيد (−19) ثم 5 بعده عبر confirm-stock القائم — إثبات أن وثيقة المسودة سليمة البنية لمسار المشتريات، residue=0

## 2026-09-08 — p363: أداء المطبخ — زمن الانتظار/التحضير وأبطأ الأطباق

**الفكرة**: شاشة المطبخ كانت تعرض «هذا الطلب تأخر» لكنها لا تجيب «هل نحن سريعون؟ وأي طبق يبطّئنا؟» — مقاييس KDS العالمية (speed of service).
- ختم زمني لكل انتقال حالة (`timestamps.preparing/served/cancelled`) في مسار تحديث الحالة الوحيد — الطلبات القديمة بلا ختم تُستبعد تلقائياً من الإحصاء
- نقطة `GET /restaurant/kitchen-stats?days=N` (1–90): تجميع `$facet` واحد على الطلبات المُقدَّمة — متوسط الانتظار (إنشاء←بدء التحضير)، التحضير، الكلي، عدد المتأخر (≥15د كعتبة الشاشة)، أبطأ 8 أطباق، وتوزيع بالساعة **بتوقيت الجزائر** (Africa/Algiers) لقرارات التوظيف
- واجهة: شريط شارات أعلى شاشة المطبخ (يُحدَّث مع الجلب الدوري) — «اليوم: N طلب — انتظار Xد — تحضير Yد — متأخر Z — أبطأ طبق: …»
- اختبار 22/22 على TEST-P363: الحالة الفارغة، الختم عند الانتقالين، ثم إرجاع الختم 30/20/5 دقائق ← انتظار 10 وتحضير 15 وكلي 25 بالضبط (±1)، متأخر=1، الطبق في قائمة الأبطأ، by_hour بساعة الجزائر، الملغى والطلب القديم بلا ختم مستبعدان، تثبيت days≤90، residue=0

## 2026-09-03 — p362: توحيد دفاتر المطعم — الدفع المباشر يُنشئ فاتورة بيع حقيقية

**الفجوة (رُصدت في p356 ووُثّقت كمرشح أول)**: مسار الدفع المباشر p336 (`/kitchen-orders/{id}/pay` من خريطة الطاولات وواجهة المطعم) كان يعلّم الطلب «مدفوعاً» **بلا أي قيد**: لا فاتورة بيع، لا خصم مخزون، لا إيداع في الصندوق — المال يُحصَّل ويختفي من الدفاتر. مسار POS+checkout كان سليماً.
- عند الدفع cash/card وبشرط عدم وجود `sale_id`: يُبنى `SaleCreate` من عناصر الطلب (أسعار DB المخزنة في الطلب، خصم الطلب p337)، يُحلّ الزبون من الهاتف (نفس مساعد pos_loyalty — هاتف جديد ينشئ زبون حد أدنى)، ثم `create_sale_op` الذرّي: فاتورة + خصم مخزون (ووصفات p303) + صندوق + إحصاءات زبون في معاملة واحدة، ويُربط `sale_id` و`invoice_number` بالطلب
- البطاقة تُقيد `bank` والنقد `cash`؛ **الآجل debt يبقى بلا قيد** (يحتاج حساب زبون إلزامياً — قرار تصميم موثّق)
- لا كسب ولاء مزدوج: الفاتورة تكسب عبر خطاف p356 الداخلي؛ مسار الكسب اليدوي القديم يبقى **احتياطياً فقط** إذا تعذّر إنشاء الفاتورة (try/except — القيد لا يُسقط الدفع أبداً)
- idempotent: إعادة الدفع على طلب مربوط لا تُنشئ فاتورة ثانية
- اختبار 27/27 على TEST-P362: فاتورة 1000 دج نقداً بزبون هاتف (ملاحظاتها «طلب مطعم KCH-…»)، مخزون 50→48، الصندوق +1000، معاملة ولاء واحدة بالضبط (100 نقطة)، الربط، إعادة الدفع بلا تكرار، الآجل بلا قيد، البطاقة ← bank، بلا هاتف ← بلا كسب وبلا كسر، residue=0

## 2026-09-03 — p361: اقتراحات الكاشير الذكية («من اشترى هذا اشترى معه»)

**الفكرة**: أكبر رفع لمتوسط السلة في الـ POS الحديث — لا يعتمد على ذاكرة الكاشير («تريد مشروباً معها؟») بل على سجل المبيعات نفسه.
- نقطة `GET /api/sales/suggestions?product_ids=…&limit=N` (أي مستخدم): تجميع co-occurrence على `sales` — المبيعات المحتوية لأي صنف من السلة (نافذة 2000 بيع أحدث، المرتجعات مستبعدة status≠returned كما في p358) ← unwind ← استبعاد أصناف السلة ← ترتيب بالتكرار ← أسماء وأسعار حية من products (الموقوف مستبعد)
- واجهة POSPage: شريط رقائق «معها غالباً:» أسفل السلة — يجلب بمهلة 350ms عند كل تغيير للسلة، نقرة تضيف الصنف عبر addProductSmart (فتنفتح نوافذ الإضافات/الوزن/السيريال كأي إضافة عادية) وتُزال الرقاقة؛ مخفي في نمط الإرجاع
- اختبار 21/21 على TEST-P361: سبعة مبيعات مدروسة (3×AB، 2×AC، 1×AD، 1×BC) ← سلة [A] تعطي B(3) ثم C(2) ثم D(1) بالترتيب الدقيق، سلة [A,B] تستبعدهما وتعطي C(3)، limit يُحترم، السلة الفارغة والمعرف المجهول ← فارغ، **إرجاع بيع A+D يُسقط D فوراً**، residue=0

## 2026-09-03 — p360: الكشك الذاتي داخل المحل (Self-Order Kiosk)

**الفكرة**: شاشة لمس عند الكونتوار يطلب منها الزبون بنفسه — يخفف طابور الكاشير ويرفع متوسط السلة (الكشك لا ينسى عرض الإضافات). يعيد استخدام بنية QR العمومية (p311/p323) لكن بلا طاولة ولا رمز.
- إعدادات `/restaurant/settings/kiosk` (مدير): تفعيل/إيقاف، اسم الكشك («كشك 1»)، الهاتف إجباري اختياري — **موقوف افتراضياً** فلا يظهر أي مسار عمومي قبل قرار المشترك
- عمومي بلا حساب: `GET /public/kiosk/{tenant}` (404 إن كان موقوفاً — القائمة + نمط الدفع) و`POST /public/kiosk/{tenant}/order` — نفس دروع QR: تسعير وإضافات من قاعدة البيانات حصراً، خنق 10 ثوانٍ (فحص الهاتف الإجباري **قبل** الخنق كي لا يحجز المحاولة الفاشلة خانته)
- طلب الكشك = طلب مطبخ `source:"kiosk"` بلا `table_id` وباسم الكشك مكان الطاولة، يحترم نمط الدفع p336 (مسبق ← pending_payment)، ويظهر في شاشة المطبخ فوراً؛ الدفع من الكاشير يعمل دون لمس (pay لا يفترض طاولة) ويكسب ولاءً بالهاتف (p356 — في الاختبار 150 نقطة)
- واجهة جديدة `KioskPage` على `/kiosk/:tenantId`: بطاقات كبيرة للمس، نافذة إضافات (إجبارية/متعددة)، سلة، اسم/هاتف، شاشة نجاح برقم طلب ضخم وعدّ تنازلي 15 ثانية يعيد الضبط للزبون التالي
- بطاقة إعدادات في خريطة الطاولات: تفعيل بنقرة، اسم الكشك، الهاتف إجباري، رابط الكشك مع نسخ وفتح
- اختبار 26/26 على TEST-P360: الإيقاف الافتراضي 404، التفعيل، القائمة العمومية بلا توكن، الهاتف الإجباري 400 بلا استهلاك الخنق، طلب بإضافة (750×2=1500 من DB)، خنق 429، إضافة غير صالحة 400، الظهور في تغذية المطبخ، الدفع بلا طاولة، كسب الولاء، الإيقاف يعيد 404، residue=0

## 2026-09-03 — p359: تنبيه انحراف كلفة الوصفات (Cost-Drift Alerts)

**الفكرة**: حين يصل شراء بسعر جديد لمكوّن، يعرف صاحب المطعم فوراً أن كلفة طبق معيّن تغيّرت — قبل أن يكتشف تآكل الهامش في نهاية الشهر.
- خدمة جديدة `services/cost_drift.py::check_cost_drift`: تستقبل تغيّرات الأسعار، تجد كل الوصفات التي تستخدم المكوّن (`components.product_id`)، تعيد حساب كلفة الطبق قديم/جديد، وعند تجاوز الانحراف **5%** تنشئ إشعاراً بصيغة الجرس (`title/message/is_read/user_id=None` ← يظهر لكل مستخدمي المستأجر) + سجل `cost_drift_log`
- الرسالة عملية: «ارتفعت كلفة «بيتزا مارغريتا» بنسبة 20% (200 ← 240 دج/وحدة) بسبب: طحين (100 ← 120). الهامش الجديد 52% — راجع سعر البيع» — والانخفاض يُشعَر به أيضاً (type=info) كفرصة هامش
- خطّافان في `purchases_routes.py`: مسار الإنشاء المباشر (التقاط purchase_price القديم قبل مزامنته) ومسار المسودة→تأكيد المخزون (`_apply_purchase_stock`) — لا تنبيه عند إنشاء المسودة، فقط عندما يلامس السعر المخزون فعلاً
- لا يُسقط الشراء أبداً: جسم الفحص كله try/except؛ أول شراء (سعر قديم=0) لا يُنبّه
- اختبار 30/30 على TEST-P359 (nt_rs_0052): +20% ← تنبيه drift=20.0 بالضبط، +1.7% ← صمت، مسودة ثم تأكيد بـ150 ← drift=23.0، خفض 150→120 ← تنبيه info بـ−20%، unread-count يشمل التنبيهات، residue=0

## 2026-09-03 — p358: هندسة القائمة (Menu Engineering) — ربح كل طبق وسرعة دورانه

**الفكرة**: الـ POS الحديث عالمياً لا يعرض «الأكثر مبيعاً» بل «الأكثر ربحاً ودوراناً» — مصفوفة القرارات.
- نقطة `GET /api/production/menu-engineering?days=N`: كل الأصناف المباعة (ليس فقط ذات الوصفات — كلفة سطر البيع، وصفةً كانت أم purchase_price، مكلفة منذ p303) ← لكل طبق: كمية، إيراد، كلفة، هامش %، شعبية %
- التصنيف بمنهج **Kasavana–Smith** الفعلي: عتبة الشعبية = 70% من النصيب العادل (1/N)، عتبة الهامش = المتوسط المرجح — نجمة/حصان/لغز/كلب + توصية عملية عربية لكل صنف (حافظ/ارفع السعر/روّج/احذف)
- واجهة: تبويب خامس «هندسة القائمة» في صفحة الإنتاج — بطاقات عدّ الأصناف الأربعة، ملخص (متوسط الهامش، عتبة الشعبية، الإيراد)، بطاقة لكل طبق بشارة ملونة وتوصية
- اختبار E2E على TEST-P358: 19/19 — أربعة أطباق مدروسة هندسياً فُرضت لتقع في الأصناف الأربعة فوقعت كلها بدقة، الحسابات (إيراد 241000، متوسط هامش 57.9%، عتبة 17.5%)، الحالة الفارغة، residue=0

## 2026-09-03 — p357: روبوت إعادة تنشيط الزبائن الغائبين (30/60/90 يوماً ← SMS)

**الفكرة**: أعلى أثر مالي مباشر في تكنولوجيا المطاعم عالمياً — كشف الزبون الغائب ومراسلته آلياً في التوقيت الصحيح.
- `services/reactivation_service.py`: كشف الخامل = آخر نشاط (بيع POS **أو** كسب ولاء — يغطي زبائن المطعم بالهاتف) أقدم من N يوماً وله هاتف؛ التشغيل يحترم التبريد (لا إزعاج) وسقف الدفعة، والنص يعوّض {name}/{company}
- `robots/reactivation_robot.py`: يمر يومياً على المستأجرين النشطين، يرسل، ويبلّغ المدراء بإشعار؛ مسجّل في robot_manager (watchdog + robot_config كبقية الروبوتات)
- نقاط `/loyalty/reactivation/{settings,preview,run}` في shipping_loyalty_routes — **ليست** في /marketing لأنها محجوبة ببوابة ecommerce_hub والتنشيط ميزة CRM لكل الأنشطة (درس: تحقق من بوابة المسار قبل اختيار ملف الراوتر)
- واجهة: تبويب رابع «التنشيط» في صفحة الولاء — تفعيل، مدة الغياب، التبريد، سقف الدفعة، نص الرسالة، معاينة الغائبين، تشغيل فوري
- معطّل افتراضياً — المشترك يفعّله ويملك النص (تكلفة SMS قراره)
- اختبار E2E على TEST-P357: 15/15 — الافتراضي معطّل، المعاينة تكشف الغائب فقط (النشط والبلا-هاتف مستبعدان)، الإرسال + سجلا reactivation_log/sms_log، التبريد يمنع التكرار، الزبون العائد يخرج من القائمة، الروبوت يقلع مع المدير، residue=0
- ملاحظة تصحيحية: المستأجرون الجدد يُسمَّون nt_<نشاط>_<رقم> (p347) — فحوص البقايا القديمة بنمط tenant_<uuid> كانت فارغة المعنى؛ اعتُمد db_name الحقيقي من saas_tenants من الآن

## 2026-09-03 — p356: ولاء الكاشير — إغلاق حلقة الكسب المكسورة (مطاعم/بيتزيريا/مقاهي + كل الأنشطة)

**الاكتشاف**: p181 بنت صرف النقاط في POS لكن **لا أحد يكسبها** — لا sales_service ولا أي مسار آخر يمنح نقاطاً؛ وواجهة المطعم p339 بلا أي ولاء.
- موديول جديد `services/pos_loyalty.py`: رصيد واحد لكل زبون (`customers.loyalty_points`)، بوابة `loyalty_settings.enabled`، كسب idempotent (معاملة earn واحدة لكل مرجع)، حلّ الزبون بالهوية ثم بالهاتف مع إنشاء زبون حد-أدنى لهاتف جديد (الكونتوار)، وفشل الولاء لا يُفشل بيعاً أبداً
- `sales_service.create_sale_op`: كسب تلقائي **بعد** commit المعاملة (خارجها حفاظاً على الذرّية) — كل بيع POS بزبون وبرنامج مفعّل يكسب `int(total × points_per_dinar)` ويرجع `loyalty_earned` في الرد
- `restaurant_routes`: `PayBody.customer_phone` + كسب عند الدفع الفعلي لطلب المطبخ (الآجل `debt` لا يكسب حتى السداد) — طلبات QR/الطاولات تحمل `customer_phone` أصلاً (p315) فتنساب تلقائياً
- الواجهة: POSPage يعرض «كسب الزبون N نقطة» ويحدّث الرصيد لحظياً؛ واجهة المطعم رُقّيت بحقل هاتف اختياري في السلة + toast الكسب عند الدفع
- اختبار E2E على TEST-P356 (نشاط مطعم): 17/17 — البوابة معطلة افتراضياً، كسب POS (100 نقطة/10000 دج)، كسب المطعم بالهاتف مع إنشاء زبون آلي، idempotency ضد الدفع المكرر، الآجل لا يكسب، الصرف p181 لا يزال يعمل (100 نقطة = 500 دج)، residue=0

## 2026-08-31 — p355: تقرير المطابقة اليومي بين النظامين + اقتراح الفصال الآمن

**الهدف**: أثناء التعايش (المشترك يعمل على القديم والمرآة تنعكس هنا) — إثبات يومي بالأرقام أن النظامين متطابقان، وكشف أي انحراف فور حدوثه، واقتراح الاستغناء عن القديم بعد 30 يوماً متطابقاً متتالياً.
- الوكيل v2.2: `compute_digest` يحسب «خلاصة الحقيقة» من قاعدة القديم نفسها يومياً (عدد/مجموع/آجل المبيعات والمشتريات لكل يوم لآخر 120 يوماً + أعداد الأساسيات) ويرسلها ضمن الدفع المعتاد (`tables.digest`) — بلا نقطة نهاية جديدة للاستقبال
- `live_sync.apply_digest`: تخزين upsert في `sync_digests` (مستند لكل يوم/نوع + مستند masters) — idempotent
- `GET /api/migration/live/reconciliation`: يقارن الخلاصة بالمخزَّن فعلياً (تجميع `$group` على created_at لكل يوم، import_source=BDV10 — يشمل المستورَد والحي معاً) ← حالة كل يوم (متطابق/فرق دقيق n/t/c) + سلسلة الأيام المتطابقة المتتالية + مقارنة الأساسيات + `cutover.suggested` عند ≥30 يوماً وأساسيات متطابقة
- الواجهة: بطاقة «مطابقة النظامين اليومية» داخل بطاقة المرآة — شارة الستريك، لافتة الفصال الخضراء، آخر 14 يوماً بحالتها، حالة الأساسيات
- اختبار E2E على TEST-P355: 23/23 — تطابق، كشف انحراف مفتعل (n+5 t+999)، إعادة تصفير الستريك، الإصلاح، 30 يوماً ← اقتراح الفصال، انحراف الأساسيات يحجب الفصال، idempotency، residue=0
- ملاحظة صدق: حساب الخلاصة في الوكيل رُوجع بنيوياً ويُختبر ميدانياً على أول جهاز rlynx حقيقي (لا Access في بيئة الاختبار) — الجانب الخادم مختبر بالكامل عبر نفس مسار الدفع

## 2026-08-30 — p354: تدريب الاستعادة الخارجية — إثبات أن نسخة B2 قابلة للاستعادة فعلياً

**السياق**: البنية موجودة وتعمل منذ p123/p128/p302 (mongodump يومي ← تشفير GPG AES256 ← رفع rclone إلى Backblaze B2 «offsite:ntcommerce-backups» يومياً 04:30 + احتفاظ 30 يوماً عن بعد + اختبار استعادة أسبوعي) — لكن الاختبار الأسبوعي يستعيد **النسخة المحلية** فقط؛ النسخة الخارجية نفسها لم تكن تُختبر من الطرف للطرف، وهذه هي الفجوة الوجودية الحقيقية (النسخة التي لا تُستعاد لا تُعَدّ نسخة).
- سكربت `scripts/offsite_restore_test.sh` الجديد (مجدول شهرياً: 1 من كل شهر 06:00، بعد الرفع الليلي والاختبار الأسبوعي): تنزيل أحدث `ntbackup_*.tar.gz.gpg` **من B2 نفسها** ← فك التشفير بنفس `.backup_key` ← تحقق إلزامي من محتويات حزمة p302 الكاملة (mongo.archive.gz + backend.env + frontend.env + docker-compose.yml) ← mongorestore لأكبر قاعدة مستأجر في `restore_test_offsite` ← مطابقة عدد الوثائق لكل مجموعة مع تقرير mongorestore ← حذف القاعدة المؤقتة وتنظيف /tmp دائماً
- التسجيل في `platform_restore_tests` بنوع `offsite_b2_restore` (مع started_at ليقرأه core/diagnostics تلقائياً) + `alert.sh` عند أي فشل في أي حلقة من السلسلة
- أول تدريب حقيقي ناجح اليوم: حزمة `ntbackup_2026-08-30_0400.tar.gz.gpg` (8.6M) نُزّلت من B2، فُكّ تشفيرها (AES256)، استُعيدت وتحققت — ok:true، بقايا=0
- لم يُمسّ أي كود backend/frontend (بنية قائمة + سكربت جديد فقط) — لا إعادة تشغيل لازمة، health:200 بعدها

## 2026-08-30 — p352: حزمة الوكيل الجاهزة لويندوز (exe بنقرة واحدة)

**الهدف**: المشترك لا يعرف Python — التثبيت يجب أن يكون بديهياً.
- الوكيل v2.1: مسارات واعية للتجميد (`getattr(sys, "frozen", False)` — الإعداد/الحالة/السجل بجانب exe عند التجميد بـPyInstaller)
- حزمة `nt_sync_agent_kit.zip` تُقدَّم من `GET /api/migration/live/agent/kit`: الوكيل + `build_exe.bat` (يثبّت pyinstaller/pyodbc ويبني exe واحد) + `install_task.bat` (يسجّل مهمة مجدولة At-Logon بصلاحيات عليا) + `README_WINDOWS.txt` (دليل عربي كامل: التثبيت، baseline، التحقق، الإبطال، استكشاف الأخطاء)
- الواجهة: زر «حزمة Windows الكاملة» بارز + زر الوكيل فقط + زر الإعداد، وتعليمات محدّثة
- ملاحظة: بناء exe يتم على جهاز ويندوز عند المشترك/الفني (PyInstaller لا يعبر المنصات) — السكربت يجعلها نقرة واحدة
- اختبار: الحزمة تُقدَّم بملفاتها الأربعة والوكيل v2.1 frozen-aware ✓ + cascade نظيف

## 2026-08-30 — p351: تعميم الترحيل على أنظمة SQL Server (PC Compta / Sage / Ciel)

**المبدأ المعماري**: الوكيل هو المعايِر الموحّد — أي نظام قديم يُترجم محلياً إلى الصيغة القانونية نفسها، والمستورِد على الخادم واحد لا يتغير.

### ما أُضيف
- **المعالج يقبل `.zip`**: حزمة قانونية ({Table}.json) ينتجها الوكيل بأمر `--dump` من أي مصدر؛ فحص بنية إلزامي (Item/Receipt/ReceiptEntry) برسالة عربية واضحة + حارس zip-slip؛ الاستيراد عبر `run_import_from_dir` (نفس خط p349 بلا mdbtools) — source_system=canonical-bundle.
- **الوكيل v2.0**: محركان — `access` (rlynx، ملف تعريف مدمج) و`mssql` (أي SQL Server: ملف تعريف تعيين حقول JSON في الإعداد — table/columns لكل جدول قانوني؛ يدعم المصادقة الموثوقة أو uid/pwd وأي ODBC driver)؛ أمر `--dump DIR` ينتج الحزمة + bundle.zip جاهزاً للرفع؛ العلامات المائية والهاش والـbaseline تعمل على كلا المحركين.
- الواجهة: قبول .zip في منطقة الرفع + توضيح مساري الاستيراد في الوصف.

### الاختبار E2E (TEST-P351)
- بناء حزمة حقيقية من BDV10.dblx (58 جدولاً، bundle.zip بحجم 5MB) ← رفعها للمعالج: **20/20 فحصاً ✓ في 24 ثانية**، bundle=True
- حزمة ناقصة ← رفض 400 برسالة تذكر الجداول الناقصة حرفياً ✓
- حذف متسلسل + residue=0 ✓
- ملاحظة صدق: محرك mssql رُوجع ونُحّي بنيوياً لكن لم يُختبر حياً — لا يتوفر خادم SQL Server في البيئة؛ أول زبون PC Compta/Sage سيكون التحقق الميداني (ملف التعريف يُضبط حسب نسخته).

## 2026-08-30 — p350: المرآة الحية للأنظمة القديمة (rlynx ← NT Commerce لحظياً)

**إكمال فكرة المالك**: المشترك يعمل على نظامه القديم وكل عملية تظهر في NT Commerce في نفس الوقت — اتجاه واحد صارم (قراءة فقط من القديم، لا كتابة عكسية إطلاقاً).

### المعمارية
- **وكيل Windows** `agents/rlynx/nt_sync_agent.py` (ملف واحد، pyodbc فقط): يقرأ ملف Access عبر ODBC على جهاز المشترك؛ علامات مائية للوصولات/المشتريات (ID تصاعدي + بنود متداخلة)، تتبع تغيّر حالة الجلسات، ومسح هاشي كامل للأصناف/الزبائن/الموردين (يلتقط تعديلات الأسعار والمخزون)؛ حالة محلية JSON + `--baseline` للتهيئة دون دفع التاريخ المستورد مسبقاً + إعادة محاولة وتوقف عند 401.
- **رمز مزامنة لكل مشترك**: `ntsync_*` يُخزَّن هاشه SHA-256 في main_db.sync_agents (حد 3 وكلاء نشطين)، المصادقة بترويسة X-Sync-Token، إبطال فوري من الواجهة.
- **الاستقبال** `POST /api/migration/live/push`: حد 5000 سطر/دفعة، تطبيق ذرّي المنطق في خيط خلفي، idempotent بـuid5 (إعادة الدفع = صفر تطبيق — مُثبت).
- **live_sync.py** (447 سطراً): نفس تعيينات p349 حرفياً + **FIFO تزايدي** — سداد الوصل الجديد يُخصَّم من أقدم فواتير الزبون المفتوحة في قاعدة NT (تحديث remaining/status/payments + قيد debt_payments)، ومرايا أرصدة الزبائن/الموردين تبقى مرجعية لعمود Account في rlynx.
- الواجهة: بطاقة «المرآة الحية» في /legacy-migration — توليد رمز (يظهر مرة)، تحميل الوكيل + ملف إعداد جاهز برمزه، حالة الوكلاء (آخر اتصال + عدادات المزامنة)، إبطال.

### الاختبار E2E (TEST-P350، بعد استيراد p349 الحقيقي كقاعدة)
19/19: دفع زبون/صنف جديدين ✓، وصل آجل 1000 ثم سداد 600 ⇒ **remaining=400 partial بالـFIFO** ✓، تعديل مخزون 42→35 ✓، جلسة يومية ✓، عدادات الحالة ✓، الإبطال ⇒ 401 ✓، الدفع المكرر idempotent ✓، الحذف المتسلسل residue=0 ✓.
