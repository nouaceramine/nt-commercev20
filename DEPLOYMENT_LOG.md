## 2026-08-30 — p349: الترحيل من الأنظمة القديمة — محوّل rlynx/BDV10 كمنتج

**فكرة المالك**: ربط قواعد بيانات الأنظمة المحاسبية القديمة بالنظام ليعمل المشترك بالتوازي حتى يتأقلم. القرار المعماري: مراحل — مستورِد ذكي أولاً (p349)، ثم مرآة حية أحادية الاتجاه لاحقاً (p350)، وممنوع أي كتابة عكسية في النظام القديم.

### ما بُني
- **اكتشاف**: قاعدة rlynx (`BDV10.dblx`) هي Microsoft Access (Jet DB) — ومحرك استيرادها كان مُثبتاً فعلياً (بيانات NT-0004 نفسها جاءت منه في p16/p159).
- `services/legacy_import/bdv_service.py` (924 سطراً): نقل أمين لمنطق p159 كخدمة معايرة لأي مستأجر — تفريغ Access عبر mdbtools (58 جدولاً ← JSON)، استيراد الأساسيات (عائلات/منتجات بإزالة تكرار الباركود/زبائن/موردون)، الحركات (مبيعات مع تخصيص ديون FIFO وأرصدة افتتاحية، مشتريات، جلسات يومية، جرد، تسويات)، ثم **تحقق آلي** (20 فحص مطابقة عدد/مجموع + حفظ الدين + عينات معمّقة + تصحيح المرايا لرصيد النظام القديم كمرجع).
- `routes/migration_routes.py` (كان stub فارغاً): `POST /api/migration/legacy/jobs` (رفع حتى 300MB + فحص ترويسة Access + حارس قاعدة غير فارغة يتطلب force + مهمة واحدة نشطة لكل مستأجر)، `GET .../jobs` و`/{id}` (تقدم حي)، `POST .../{id}/rollback` (حذف كل ما استُورد — import_source=BDV10 فقط).
- المهمة تُنفَّذ خلفياً (run_in_executor) وحالتها في قاعدة المستأجر — تصمد أمام عمّال uvicorn الأربعة وإعادة التشغيل.
- الواجهة `LegacyMigrationPage.js` على `/legacy-migration`: منطقة رفع، شريط تقدم حي بالخطوة العربية، جدول تقرير المطابقة، زر تراجع بتأكيد، سجل العمليات. عنصر قائمة «الترحيل من نظام قديم» في قسم الإعدادات.

### البنية التحتية
- Dockerfile: `+ mdbtools` (قراءة Access داخل الحاوية)؛ docker-compose: volume `./legacy_uploads:/legacy_uploads`؛ nginx: `client_max_body_size 25m→100m` (الملف الحقيقي 46MB) + نسخة repo محفوظة. إعادة بناء الحاوية وإقلاع نظيف.

### الاختبار E2E (مستأجر TEST-P349 + BDV10.dblx الحقيقي 46MB)
- رفع عبر API ← تفريغ 58 جدولاً ← استيراد كامل في ~30ث: **20/20 فحصاً ✓، all_ok=true**
- المنتجات 7412/7412، المبيعات 36695/36695 (مع الأرصدة الافتتاحية)، حفظ الدين متحقق، 5/5 عينات معمّقة، 20 مرآة رصيد صُحّحت
- التراجع: حذف 7412 منتجاً + 36695 بيعاً + كل الحركات — القاعدة فارغة بعدها
- حذف متسلسل للمستأجر التجريبي: **residue=0** (قاعدة + ملفات مرفوعة)
- الحزمة main.dbe745d2.js منشورة والموقع 200

## 2026-08-29 — p348: خطة معالجة الأخطاء الشاملة + تطهير سجل الأخطاء + تحليل خبير

**طلب المالك**: «نفّذ خطة المعالجة p348، واحذف الأخطاء من سجل الأخطاء بعد معالجتها، وبعد الانتهاء أعد تحليلاً شاملاً للنظام والوحدات واكتشاف الأعطال والأخطاء البرمجية وكل ما تراه كخبير برمجيات مناسباً».

### الإصلاحات الحية (8 أصناف أخطاء)
1. **لوحة الكتالوج العامة KeyError** (display_routes.public_catalog_board): منتج قديم بلا حقل `id` كان يُسقط اللوحة كلها → تخطّي العناصر الناقصة بـ `p_.get("id")`. اللوحة تعود 200 على NT-0004 (1000 عنصر).
2. **external_id فارغ يصطدم بالفهرس الفريد** (ecom/leads + ecom/webhooks): أدخلان سجلات بـ `external_id=""` فرفض الفهرس الفريد (channel, external_id) الثانية → معرّف اصطناعي فريد `manual-<id12>`/`auto-<id12>` عند الغياب.
3. **أكواد الجلسات اليومية تكرارية** (daily_sessions_routes): كود العميل يُؤخذ كما هو → ضمان ذرّي عبر `next_code("S",3,year)` عند الغياب/التكرار + التفاف إدراج على DuplicateKeyError بإعادة سحب مرة واحدة. اختبار: S900 محفوظ، S901 ذرّي عند التكرار، S902 عند الفراغ.
4. **تكرار كود البيع تحت التزامن** (sales_service.create_sale_op): المعاملة ACID كاملة تُعاد في حلقة محاولة×2 مع إعادة سحب كود ذرّي عند DuplicateKeyError (التكرار داخل معاملة حقيقية مستحيل الالتقاط داخلياً). اختبار: بيع مكرر الكود ⇒ 201 بـ BV0001/26 بدل 500.
5. **إنذارات watchdog الكاذبة** (module_watchdog): مهلة probe 8ث→25ث — كانت ReadTimeout تُسجَّل أعطالاً أثناء ذروة النسخ الاحتياطي. النتيجة: 44/44 وحدة ok.
6. **تصالح الفهارس عند الإقلاع** (main.py + enhanced_shipping_indexes): فحص المواصفة الفعلية قبل الإنشاء — ecom_leads channel_1_external_id_1 (يُسقط إن كان بلا partialFilterExpression) وecom_shipping_labels tracking_number_1 (يُسقط إن كان غير فريد) + فرع «نفس الاسم» يعيد الإنشاء بلا IndexKeySpecsConflict. إقلاع نظيف: صفر تحذيرات فهارس.
7. **تحقق بندَي p348 (NameError sim/transfer، مسارات المطعم العامة، ObjectId المحاسبة)**: مُصلحة سلفاً في p196/p322/p323 — وُثّق التحقق بدل إعادة التنفيذ.
8. **واجهة ReferenceError/522**: لا تكرار منذ 08-18/19 — أُغلق توثيقياً.

### الجذر الأعمق: قواعد «شبحية» tenant_* (فات p347)
- 10 روبوتات + 6 خدمات كانت تشتق `f"tenant_{tid}"` أو ترشّح `startswith("tenant_")` مباشرة → أعادت إنشاء **28 قاعدة legacy فارغة** بعد ترحيل p347 (Mongo يُنشئ القاعدة كسولاً عند أي كتابة)، وكانت تحسب تقارير من بيانات **فارغة** وتكتب نفايات auto_reports.
- الإصلاح: `TENANT_DB_PREFIXES=("nt_","tenant_")` + `is_tenant_db()` في core/db_naming، وتحويل كل المواقع: report/customer/debt/pricing/maintenance/inventory/data_integrity (resolve_db_name)، notification/prediction/profit (is_tenant_db)، migrations_runner (كانت تهجر ترحيلات nt_*!)، backup_routes (كانت النسخ الاحتياطية تفوّت nt_*!)، diagnostics، db_tree، code_generator (ختم nt_)، identity_migration، fix_permissions.
- أدلة جنائية عبر oplog (local.oplog.rs): آخر إنشاء كان في فجوة drop→restart مع الخلفية نصف المرقّعة؛ بعد الإسقاط النهائي 01:34:37: **صفر إعادة إنشاء** 13+ دقيقة، وتقارير prediction/profit حقيقية تُكتب الآن في nt_rt_0004.
- 28 قاعدة legacy أُسقطت بعد تحقق محتوى (كل مجموعاتها غير auto_reports فارغة) + نفايات auto_reports نُظّفت.

### تطهير سجل الأخطاء (بموجب توجيه المالك)
- 17 نتيجة autoheal مُعالجة ⇒ status=resolved مع ملاحظة حلّ عربية وresolved_by=p348.
- system_errors: 8 ⇒ **0** (كل المدخلات المعالجة حُذفت).
- نتائج نشطة متبقية (تجارية/تشغيلية، ليست أعطال كود): هبوط مبيعات NT-0004، تنبؤ نفاد مخزون، اتجاه هابط، فرق صندوق S002/26، تنبيه CPU قديم من 08-26 (الحمل الآن 0.06 — عابر).

### التحليل الشامل (خبير)
- **القواعد**: 62⇒34 (28 nt_* + رئيسيات)؛ صفر أيتام/مفقودة؛ كل المستأجرين الـ28 لهم db_name.
- **سلامة الكود**: ast.parse على كل الملفات الـ26 المعدّلة قبل الكتابة؛ إقلاع نظيف؛ صفر tracebacks في السجلات.
- **السباقات**: صفر أكواد مكررة في sales/daily_sessions؛ عدّادات _code_counters سليمة؛ outbox/DLQ backlog=0.
- **الصحة**: حمل 0.06، قرص 42%، RAM 4.5/8GB، 7/7 حاويات up منذ 5 أيام، الحزمة main.520d28dc.js حية، .backup_key و.env موجودان.
- **انحدارات**: p341 (14/14) + p342 (ALL PASS) بعد كل التغييرات.

## 2026-08-28 — تفعيل إشعارات تليغرام للمالك (إكمال p344)
- بوت ntcommerce_alerts (@ncramine_bot) مربوط بمحادثة المالك (chat_id=5152573605)؛ حُذف webhook خارجي غريب (kasperiq.top — خدمة Twasl قديمة معطلة) قبل الربط
- الحفظ عبر PUT /api/saas/owner-notifications + اختبار POST /test ⇒ telegram=true؛ on_error/on_fixed مفعّلان — أعطال الوحدات الـ44 وتعافيها يصلان تليغرام لحظياً
- تم لاحقاً: المالك أبطل التوكن القديم (/revoke) واستُبدل بتوكن نظيف (…f1fQ) — التوكن المكشوف سابقاً ميت نهائياً؛ الاختبار بالجديد telegram=true

## 2026-08-28 — p347: تسمية قواعد بيانات المشتركين حسب النشاط (nt_<code>_<seq>)

**طلب المالك**: «أريد قواعد بيانات المشتركين بأيدٍ حسب نوع النشاط — نأخذ أول حرفين من اسم النشاط التجاري فتصبح مرتبة». نُفّذ بصيغة محسّنة: رموز نشاط منسّقة فريدة (بدل أول حرفين حرفياً لتفادي التصادم) + تسلسل رقمي ثابت.

### الصيغة
- `nt_<code>_<seq>` — مثال: `nt_rs_0025` (مطعم/بيتزا NT-0025)، `nt_rt_0004` (المستأجر الحقيقي Nouacer Telecom، تجزئة retail)
- 23 رمز نشاط منسّقاً في `ACTIVITY_CODES` (rs=restaurant, rt=retail, ws=wholesale, ph=pharmacy, ...) + `gn` الافتراضي
- الرقم التسلسلي = أرقام short_id — فريد ببنيوياً، لا تصادم ممكن

### core/db_naming.py (جديد)
- `legacy_db_name(tid)` = الصيغة القديمة `tenant_<uuid>`؛ `build_db_name(short_id, business_type)` = الصيغة الجديدة
- `resolve_db_name(tid)`: **حلّال مركزي وحيد** — كاش محلي (300ث) ← Redis `tenant_db_name:{id}` (دائم) ← fallback قديم (كاش 30ث)؛ يعمل عبر عمال uvicorn الأربعة
- `register_db_name` (عند الإنشاء) + `load_all_db_names(main_db)` عند بدء التشغيل (main.py startup)

### الترقيع (16 ملفاً — 27 موقع اشتقاق)
- `config/database.py`: `get_tenant_db` يمر عبر resolve_db_name — **كل** الكود القديم يحصل على الاسم الجديد تلقائياً
- مواقع الاشتقاق المباشر `f"tenant_{...}"` كلها استُبدلت: database_mgmt, data_browser, tenants, account_control, id_audit, monitoring, system_sync, auth_users (login/me), ecom webhooks + shipping_webhook, create_enhanced_indexes, tenant_template, db_tree, module_watchdog
- **3 مسارات إنشاء** تحسب short_id + db_name وتخزّنه في وثيقة المستأجر وتسجّله في Redis قبل نسخ القالب: saas/tenants (سوبر أدمن)، agent_self_service (وكيل)، registration (تسجيل ذاتي)

### الترحيل الفعلي (28/28 قاعدة)
- نسخ احتياطي أولاً: `backups/p347/dbdumps/*.archive` (mongodump لكل الـ28 قاعدة، 60MB) + نسخ الملفات الـ16
- آلية: `renameCollection` لكل مجموعة (نقل سريع، الفهارس تتبع) مع تحقق عدّ الوثائق لكل مجموعة
- تجربة أولاً على 27 قاعدة تجريبية، ثم **NT-0004 آخراً** بعد إثبات العملية (بموافقة المالك: «لا تفضيل») — بياناته سليمة: **36708 مبيعات / 7415 منتجاً** بعد الترحيل، والحلّال يطابق الوثائق (MATCH)
- **صفر قواعد `tenant_*` قديمة متبقية** — `MIGRATION CLEAN`

### الاختبارات
- p341 (إنفاذ البوابات 14/14) + p342 (التوحيد) بعد الترحيل: **ALL PASS** — لا انحدار
- E2E: إنشاء TEST عبر API ⇒ NT-0034 بقاعدة `nt_rt_0034` تلقائياً ⇒ دخول unified-login **200** ⇒ حذف متسلسل يسقط القاعدة (residue=0)
- ملاحظة توثيقية: `/api/auth/login` محجوز لسوبر الأدمن (simple_auth يُسجَّل أولاً)؛ المستأجرون يدخلون عبر `/api/auth/unified-login` — نتيجة 401 الأولى في الاختبار كانت خطأ في نقطة النهاية المختبَرة لا انحداراً

### البنية بعد p347
```
nt_rs_0025 (مطعم)  nt_rt_0004 (تجزئة — Nouacer Telecom)  nt_ph_0010 (صيدلية)  ...
كل قاعدة جديدة تُسمّى تلقائياً حسب نشاط خطة/نموذج الإنشاء
```
...
### التغييرات
- **frontend/src/pages/RestaurantPOSPage.js (جديد، مسار `/restaurant-pos`)**: وضعان — **طاولات** (شريط طاولات أفقي: اسم + مشغولة/فارغة، الإلحاق بالطلب المفتوح تلقائي من الخلفية) و**سريع** (فاست فود بلا طاولة: «دفع كاش» ينشئ الطلب ويؤكد دفعه في خطوة، أو «للمطبخ فقط»). شبكة أصناف مصوّرة بأهداف لمس كبيرة (min-h 110px)، بحث فوري (اسم عربي/لاتيني/باركود)، فلترة بالعائلات، **الأكثر مبيعًا أولًا** مع أيقونة Flame، منبثق الإضافات (modifier groups) بنفس منطق واجهة النادل، سلة ثابتة سفلية، حوار فاتورة الطاولة (إجمالي/صافٍ بعد الخصم + خصم %/مبلغ + تأكيد دفع كاش + إنهاء وتحرير). احترام نمط الدفع المسبق (p336) بتنبيه نصي
- **backend/routes/restaurant_routes.py**: نهاية جديدة GET `/restaurant/top-sellers?days=30` — تجميع kitchen_orders (بلا الملغاة/المجدولة) لترتيب الشبكة
- **Layout.js**: عنصر «POS المطعم» في قسم المطعم

### التحقق
- ast + esbuild ✓؛ إعادة تشغيل + health ✓؛ top-sellers يرجع ترتيبًا حقيقيًا من طلبات الاختبار السابقة قبل تنظيفها ✓
- Playwright: /restaurant-pos يفتح، التبديل طاولات/سريع، إضافة صنف للسلة (مع مسار الإضافات عند وجودها)، الشبكة والبحث ✓ (18/18 ضمن الجولة الموحدة)
- البناء main.69b94a0c.js + النشر release 20260827_211855 ✓

## 2026-08-28 — p338: شاشة المطبخ الاحترافية — مصدر الطلب + طبع تلقائي

### التغييرات
- **KitchenDisplayPage.js**: شارة مصدر ملونة على كل تذكرة (QR زبون/نادل/كاشير/مطبخ/توصيل/جار) + أزرار فلترة بالمصدر + شارات غير مدفوع/مدفوع؛ عرض الإضافات بأخضر «+ خيار» والملاحظات بأحمر؛ **طبع تلقائي** اختياري (تبديل يُحفظ في localStorage) — تذكرة مخفية `#kds-ticket` تُطبع عبر window.print مع CSS @media print فور وصول طلب جديد عبر SSE
- المصدر يُخزَّن منذ p336 في `source` (pos/waiter/kitchen/qr/delivery/neighbor)

### التحقق
- Playwright: kds-autoprint + أزرار kds-src-* موجودة والصفحة تعمل ✓

## 2026-08-28 — p337: خصومات طلبات المطعم + الطلبيات المجدولة

### التغييرات
- **خصومات**: POST/DELETE `/restaurant/kitchen-orders/{id}/discount` — نسبة (0–100%) أو مبلغ (≤ الإجمالي) أو **كوبون** من وحدة promotions (code + discount_type/discount_percent)؛ `_order_out` يرجع `total` + `discount_amount` + `final_total`؛ يرفض الخصم على طلب منتهٍ
- **مجدولة**: KitchenOrderCreate يقبل `scheduled_for` (ISO مستقبلي) + `remind_days` (1 أو 3) → الحالة `scheduled`: لا تدخل المطبخ ولا تشغل طاولة؛ **ترقية تلقائية** إلى pending عند حلول الموعد (في list_kitchen_orders)؛ GET `/restaurant/scheduled-orders` (لوحة قيد التجهيز مرتبة زمنيًا + due_soon)؛ POST `/scheduled-orders/{id}/activate` للإرسال الفوري؛ **تذكير تلقائي** عند بلوغ نافذة التذكير: إشعار داخلي للمدراء + بريد Resend للمالك إن توفر (مرة واحدة reminder_sent)
- **TablesMapPage.js**: بطاقة «طلبيات مجدولة» (قائمة + تفعيل + تسجيل طلبية: موعد + تذكير + هاتف + ملاحظات + منتقي أصناف) + واجهة خصم في حوار الطاولة (نسبة/مبلغ/كوبون + إزالة + إجمالي مشطوب وصافٍ أخضر)

### التحقق (E2E في الحاوية، مستأجر البيتزيريا)
- نسبة 10% على 1000 → صافٍ 900 ✓؛ مبلغ 250 → 750 ✓؛ 150%/مبلغ أكبر/كوبون وهمي مرفوضة (400/400/404) ✓؛ كوبون TEST-P337 بنسبة 25% → 750 ✓؛ إزالة الخصم ✓
- مجدولة للغد: مخفية عن المطبخ (?all=1 أيضًا)، ظاهرة في scheduled-orders ✓؛ موعد ماضٍ/remind_days=2 مرفوضان ✓؛ activate → pending وتظهر في المطبخ ✓؛ طلبية منتهية الموعد تُرقّى تلقائيًا ✓؛ التنظيف residue=0 ✓

## 2026-08-28 — p336: توفيق الدفع المسبق/اللاحق + الفاتورة الموحدة للطاولة

### التغييرات
- **إعدادات**: GET/PUT `/restaurant/settings/orders` — `payment_mode: postpaid` (افتراضي، سلوك ما قبل المرحلة حرفيًا) | `prepaid`
- **prepaid**: الطلبات الجديدة (موظفون + QR) تُنشأ `pending_payment` + `payment_status=unpaid` — **مخفية عن المطبخ** حتى تأكيد الدفع عبر POST `/restaurant/kitchen-orders/{id}/pay` (cash/card/debt)؛ الإلحاق بطلب مفتوح يشمل pending_payment ويعيد ضبط unpaid؛ checkout/تحصيل التوصيل يضبطان paid + paid_at
- `_order_out` يحسب `total` دومًا؛ list_kitchen_orders يقبل `?all=1`
- **TablesMapPage**: بطاقة نمط الدفع + شارة غير مدفوع/مدفوع وإجمالي على بطاقة الطاولة + صف فاتورة وزر «تأكيد الدفع (كاش)» في الحوار
- **QrMenuPage**: شاشة النجاح في النمط المسبق: «سجّلنا طلبك — أكّد الدفع عند الكاشير»

### التحقق (E2E)
- postpaid بلا تغيير (pending مباشر)؛ الإلحاق يدمج الكميات في فاتورة واحدة (سطران، 2400) ✓؛ prepaid: إخفاء/إظهار عبر الدفع ✓؛ إلحاق بعد الدفع يعيد unpaid ✓؛ pay cash ✓؛ إعادة الإعدادات لـ postpaid + تنظيف كامل ✓

## 2026-08-28 — p335: الجيران B2B — طلبات المحلات المجاورة بأسعار خاصة ودين

### التغييرات
- **backend**: نماذج NeighborIn/Update/Settle/OrderCreate؛ إدارة كاملة GET/POST `/restaurant/neighbors` + PUT/DELETE `/{id}` + POST `/{id}/settle` (تسديد يُنقص الرصيد ويُؤرشف في neighbor_settlements) + GET `/{id}/statement?days` (طلبات + تسديدات + رصيد)؛ التسعير `_nbr_price`: سعر خاص بالمنتج ← ثم خصم عام % ← ثم الأساس؛ عمومي: GET `/restaurant/public/neighbor-menu/{tenant}/{token}` (قائمة بأسعار الجار) + POST `/restaurant/public/neighbor-order/{tenant}/{token}` (خنق 10ث، ينشئ طلب مطبخ source=neighbor باسم المحل كطاولة، payment_status=debt يُضاف للرصيد أو unpaid للكاش)
- **frontend**: صفحة `/neighbors` (إنشاء/تعديل/حذف + نسخ رابط الطلب + كشف حساب وتسديد) + صفحة عمومية `/b2b/:tenantId/:token` (قائمة مصورة بالأسعار الخاصة والسعر الأساسي مشطوبًا، سلة، حقل «من يطلب؟»، شاشة نجاح تشرح الدين/الكاش) + عنصر «الجيران B2B» في قسم المطعم

### التحقق (E2E)
- إنشاء جار → قائمة بأسعار خاصة (سعر خاص 1000 + خصم 10% على الثاني) ✓؛ طلب دين KCH-… بمصدر neighbor والرصيد يتراكم (3350) ✓؛ تسديد 500 → 2850 ✓؛ رمز وهمي → 404 ✓؛ Playwright: إنشاء/حذف جار عبر الواجهة + طلب كامل من صفحة /b2b العمومية بنجاح ✓؛ التنظيف residue=0 (شمل جارَين متسربَين من اختبارات سابقة) ✓

## 2026-08-28 — p334: روابط التواصل في صفحة QR + مشاركة الطاولة

### التغييرات
- **backend**: GET/PUT `/restaurant/settings/social` (instagram/facebook/tiktok/google_maps/whatsapp/website — تحقق http(s))؛ قائمة الطاولة العامة ترجع الروابط غير الفارغة فقط
- **QrMenuPage**: بطاقة «تابعنا» بأيقونات المنصات + زر **مشاركة الطاولة** (navigator.share للرابط الدائم /r/{tenant}/{table} مع نسخ احتياطي) — في شاشة النجاح وأسفل القائمة؛ **TablesMapPage**: بطاقة تحرير الروابط

### التحقق
- رابط غير صالح → 400 ✓؛ الروابط تظهر في table-menu العامة ✓؛ Playwright: qr-social + qr-share-table ظاهران ✓؛ تنظيف كامل ✓

## 2026-08-28 — p333: شاشات TV — تحديد المنتجات لكل شاشة + تقسيم 4/6/8

### التغييرات
- **backend**: ScreenUpdate يقبل `product_ids` (≤200، إزالة تكرار) و`grid_layout` ∈ 0/1/4/6/8 (غيرها → 400)؛ إعداد الشاشة العامة يعرضهما؛ catalog-board يقبل `?screen=<token>` فيفلتر المنتجات ويرجع التقسيم
- **frontend**: TvCatalogPage يدعم التقسيم (شبكة ملء الشاشة 1/4/6/8 مع تدوير الصفحات كل 8ث وشارات التوفر)؛ TvHubPage يمرر رمز الشاشة؛ **ScreensPage**: لكل شاشة catalog/slider منتقي تقسيم + لوحة «تحديد المنتجات» (بحث + مربعات اختيار + حفظ)

### التحقق (E2E)
- شاشة TEST على البيتزيريا: grid 8 + منتجان → الإعداد يعرضهما واللوحة العامة مفلترة تمامًا ✓؛ grid=5 → 400 ✓؛ إصلاح KeyError في catalog-board (إسقاط id من الإسقاط) ✓؛ تنظيف الشاشات التجريبية residue=0 ✓

## 2026-08-28 — p332: تحقق — تسجيل شاشة الكاشير (DVR/NVR) معمّم فعلًا لكل الأنشطة

- الوحدة (p281) متعددة الأجهزة ومتاحة للكل من الإعدادات بلا قيد ميزة — لا حاجة لأي كود
- E2E على مستأجر bida (غير مطعم): إنشاء جهاز + حذفه عبر API ✓ residue=0 — مرحلة توثيقية فقط

## 2026-08-28 — p330: فلترة المشتركين حسب النشاط في /saas-admin/subscribers

### التغييرات
- **SubscribersPage.js**: قائمة منسدلة بجانب البحث — «كل الأنشطة» + 23 نشاطًا من business_profiles **مع عداد المشتركين لكل نشاط**؛ الفلترة تندمج مع البحث النصي (business_type، الافتراضي retail)

### التحقق
- Playwright: tenants-activity-filter موجودة وتعمل ✓
...
### التغييرات
- **طلب المستخدم**: الرمز المطبوع على الطاولة يجب أن يبقى ثابتًا للأبد؛ المسح ينشئ رابط طلب جديدًا يُحذف بعد الدفع
- **Backend** (routes/restaurant_routes.py): GET `/restaurant/public/table-session/{tenant}/{table}` — يرجع رابط الطلب المؤقت الحالي (qr_token) أو ينشئه كسلًا؛ الدوران نفسه من p323 يبقى (تسليم/إلغاء/دفع/يدوي)
- **Frontend**: QrMenuPage — زيارة بلا رمز (المسح من الرمز المطبوع) تجلب الجلسة وتستبدل العنوان بالرابط المؤقت (navigate replace)؛ الرسالة المنتهية صارت «امسح رمز الطاولة من جديد»؛ TablesMapPage — الـQR المطبوع والنسخ صارا بالرابط الدائم بلا رمز + ملاحظة «اطبع مرة واحدة»

### التحقق (E2E كامل)
- دورة حياة عبر curl: مسح1 (90bcd60a87) → طلب KCH-20260827-0001 → تسليم ⇒ الرابط القديم 410 → مسح2 ⇒ رمز جديد (7066a1cf12) → قائمة 200؛ طاولة غير موجودة 404 برسالة عربية
- Playwright (موبايل، المجال العام): `/r/tid/tbl` الدائم ⇒ تحويل تلقائي للرابط المؤقت والقائمة تظهر؛ رمز قديم ⇒ «انتهت صلاحية رابط الطلب — امسح رمز الطاولة من جديد»
- التنظيف: الطلب الاختباري حُذف + _qr_throttle=0 (الطلبات المقدَّمة من 2026-08-26 بقيت — من تجربة المستخدم نفسه)
- Deploy: release 20260827_003119، main.ef34fab0.js حي

## 2026-08-27 — p324: إصلاح صفحة «Not Found» لروابط QR القديمة

### التغييرات
- **المشكلة (أبلغ عنها المستخدم بالصورة)**: نسخ رابط QR من صفحة محمّلة قبل p323 (بصيغة p311 بدون رمز) → الهاتف فتح `/r/tid/tbl` → الجلب بمسار بمقطع رمز فارغ → 404 الافتراضي من FastAPI («Not Found») يظهر كنص خام إنجليزي للزبون
- **الحل** (frontend/src/pages/QrMenuPage.js فقط، صفحة موبايل الزبون): بلا رمز ⇒ رسالة الانتهاء مباشرة دون جلب؛ وأي 404/410 أو detail «Not Found» يُترجم إلى «انتهت صلاحية رابط هذه الطاولة — اطلب من النادل الرمز الحالي»

### التحقق
- esbuild OK، build نظيف (تحذيرات source-map قديمة)، Deploy release 20260826_222929، main.ce45db08.js حي
- Playwright (موبايل 390×844، المجال العام): `/r/tid/tbl` بلا رمز ⇒ الرسالة العربية ولا أثر لـ«Not Found»؛ `/r/tid/tbl/90bcd60a87` الصحيح ⇒ القائمة بالأطباق ✓

## 2026-08-27 — p323: روابط QR مؤقتة للطاولات

### التغييرات
- **المشكلة**: رابط QR لكل طاولة كان ثابتًا — من صوّره مرة يطلب متى شاء
- **الحل**: `qr_token` على وثيقة الطاولة؛ يدور تلقائيًا عند تحريرها (تسليم/إلغاء الطلب)، عند الدفع، ويدويًا بزر «تجديد الرابط» في خريطة الطاولات. الطاولات القديمة تُرقَّى كسلًا عند أول تحميل
- **Backend** (routes/restaurant_routes.py): qr_token عند إنشاء الطاولة + ترقية كسولة في list_tables، دوران في update_order_status (served/cancelled) و checkout_table، POST /restaurant/tables/{id}/rotate-qr، نهاية عامة GET /restaurant/public/table-menu/{tenant}/{table}/{token} (410 عند رمز قديم)، والطلب العام /qr-orders يتحقق من الرمز أيضًا
- **Frontend**: QrMenuPage (مسار /r/:t/:tbl/:token + صفحة «انتهت صلاحية الرابط»)، TablesMapPage (زر تجديد data-testid table-qr-rotate + ملاحظة أن الرابط مؤقت)، App.js (مساران: القديم بدون رمز يعرض صفحة الانتهاء)

### التحقق (E2E كامل)
- دورة حياة: إنشاء طاولة (رمز تلقائي) → قائمة 200 → طلب KCH-20260826-0001 ناجح → رمز خاطئ 410 (قائمة + طلب) → تسليم الطلب ⇒ الطاولة حرة والرمز دار → الرابط القديم 410 والجديد 200 → تجديد يدوي ⇒ رمز جديد مختلف → residue=0 (الطاولة الاختبارية نُظّفت)
- Playwright (موبايل 390×844): قائمة العميل تعرض الأطباق بالرمز الصحيح؛ الرابط القديم يعرض «انتهت صلاحية رابط هذه الطاولة»
- Deploy: release 20260826_204225، main.7737d8f9.js حي — commit de3ff13
...
### التغييرات
- backend/routes/restaurant_routes.py — حمولة menu-board تشمل معرض الصور (images، حتى 5) لكل منتج
- frontend/src/pages/TvMenuPage.js — وضع شرائح ?mode=slider: صور الأطباق المتوفرة بملء الشاشة، تبديل كل 8 ث بتلاشٍ، اسم/سعر/«بقي N»، نقاط تقدم؛ النافد يُستبعد تلقائيًا؛ بلا صور → يرجع للشبكة
- بيانات NT-0025: طبق ثانٍ «بيتزا الدجاج» (1000 دج، وصفة بتكلفة 458 دج) + 3 صور أطباق ذاتية الاستضافة /api/static/uploads/dishes/ (مارغريتا بصورتين، دجاج بصورة)

### التحقق
- ast.parse + esbuild ✓، health 200 بعد إعادة التشغيل ✓
- curl menu-board: طبقان متاحان، الباقي 33 لكلٍّ، img+gallery في الحمولة ✓
- Playwright @1920px: الشبكة تعرض البطاقتين بالصور ✓؛ الشرائح: دوران مؤكد (دجاج ← مارغريتا/خضار) مع الاسم والسعر ✓
- نشر: release 20260826_193015 — main.fbeaa66b.js + 2337.9d2cfeb1.chunk.js مؤكدان ✓

---

## 2026-08-27 — p320: شاشة تلفاز المنتجات بالمخزون الحي (/tv/menu/:tenantId)

### التغييرات
- backend/routes/restaurant_routes.py — نقطة عمومية جديدة GET /restaurant/public/menu-board/{tenant_id}: منتجات قابلة للبيع (سعر>0، غير محظورة) مع توفر حي: طبق بوصفة → دفعات = min(مخزون مكوّن ÷ كميته)؛ غير مخزوني بلا وصفة → متاح دائمًا؛ مخزوني → الكمية>0. أسماء/أسعار/توفر فقط (بلا تكاليف)
- frontend/src/pages/TvMenuPage.js (جديد) — شاشة تلفاز داكنة ملء الشاشة: ترويسة باسم المطعم + ساعة، بطاقات كبيرة، «بقي N» عند ≤10، قسم «نفذت حاليًا» بشارة «نفذت الكمية» وسعر مشطوب، تحديث كل 10 ث
- frontend/src/App.js — مسار عام /tv/menu/:tenantId
- docs/RESTAURANT_GUIDE.md — قسم شاشات التلفاز (لوحة الطلبات p314 موجودة أصلاً على /board/:tenantId)

### التحقق
- ast.parse + esbuild ✓، إعادة تشغيل الخلفية health 200 ✓
- curl NT-0025: بيتزا مارغريتا متاحة، الباقي 33 = min(40 عجينة، 33 جبن، 37 صلصة) ✓
- Playwright @1920px: البطاقة تظهر ← تصفير مخزون العجينة مؤقتًا ← بعد دورة تحديث انتقلت لـ«نفذت حاليًا» بشارة حمراء ✓ ← استعادة المخزون 10 ← عادت متاحة (residue=0) ✓
- نشر: release 20260826_190245 — main.abdc7ef8.js + المقطع الكسول 2337.bac0a101 مؤكدان على nt-commerce.net

---

## 2026-08-26 — p317: فحص انحدار شامل + إصلاح تكلفة الإضافات في COGS + دليل تشغيل المطعم
---
## 2026-08-26 — p319: تجهيز مثال بيتزيريا كامل في NT-0025 (مكوّنات + شراء + وصفة + إضافات)

### البيانات المُنشأة (بطلب المالك — حساب تجريبي دائم، تُبقى ولا تُنظَّف)
- 4 مكوّنات مخزونية: عجينة بيتزا (200 دج/كغ)، جبن موزاريلا (1200)، صلصة طماطم (600)، شرائح دجاج (1500)
- فاتورة شراء PUR-20260826-0001 (مورد عام، مدفوعة نقداً 15800 دج) → المخزون: 10/5/3/4
- طبق نهائي «بيتزا مارغريتا» (غير مخزوني، سعر 800 دج)
- وصفة: 1 بيتزا = عجينة 0.25 + جبن 0.15 + صلصة 0.08 → التكلفة الحية 278 دج، هامش 65.2%، FC 34.8%
- مجموعة إضافات «إضافات» (حتى 3): جبن إضافي +100 (خصم جبن 0.05)، دجاج إضافي +150 (دجاج 0.08)، صلصة إضافية +50 (صلصة 0.03)

### التحقق (Playwright على الإنتاج)
- صفحة الإنتاج تعرض بطاقة الوصفة بالتكلفة الحية والهامش ✓
- حوار «الإضافات» يعرض الخيارات الثلاثة مربوطة بمكوّنات المخزون ✓
- لا تغيير كود — بيانات فقط. نسخة احتياطية: backups/p319/

---
## 2026-08-26 — p318: إصلاح المسح الصامت لميزات الأنشطة (restaurant/production/rental) + إعادة تفعيل NT-0025

### السبب الجذري (بلاغ المالك: «حساب البيتزيريا التجريبي لا يُظهر أي تغيير»)
- حوار «الميزات» في صفحة المشتركين كان يستبدل features_override استبدالاً كلياً بمفاتيح SUPPORTED_FEATURES الـ16 فقط
- المفاتيح الاختيارية restaurant/production/rental لم تكن ضمن القائمة → أي حفظ للحوار كان يمسحها صامتاً من المستأجر
- NT-0025 فقد restaurant/production بهذه الطريقة رغم تطبيق ملف المطعم عليه في p307

### التغييرات
- backend/routes/saas/tenants_routes.py — SUPPORTED_FEATURES += restaurant/production/rental (لن تُمسح عند الحفظ بعد الآن)
- frontend/src/pages/admin/saas/SubscribersPage.js — ALL_FEATURES += 3 مفاتيح اختيارية مرئية: 🍕 وضع المطعم / 🏭 الإنتاج / 🔑 الكراء (قابلة للتعديل من حوار الميزات)
- إعادة تطبيق ملف «مطعم/بيتزيريا» على NT-0025 → restaurant:true + production:true (تدقيق: 26 سليم / 2 منحرف — NT-0004 محمي وKadi.multimedia متروك للمالك)
- نشر الواجهة: release 20260826_162658 — main.b7607221.js مؤكد على nt-commerce.net

### التحقق (Playwright على الإنتاج، مستخدم demo-restaurant@nt-commerce.net)
- /auth/me يرجع restaurant:true + production:true ✓
- الشريط الجانبي يعرض قسم «المطعم» (شاشة المطبخ/خريطة الطاولات/واجهة النادل/طلبات التوصيل) + قسم «الإنتاج» ✓
- صفحات /kitchen و/delivery و/production تعمل ✓
- ملاحظة تشغيلية: الشريط الجانبي يفتراضياً مطوي (أيقونات فقط) — زر التوسيع أعلى الشريط يُظهر العناوين


### التغييرات
- backend/services/application/sales_service.py — **إصلاح فجوة محاسبية من p308**: الإضافة المرتبطة بمخزون كانت تُخصم كميةً فقط دون قيمة → قيد المخزون (380) ينحرف عن الواقع؛ الآن تكلفة الإضافة (qty × سعر شراء المكوّن الحالي) تُضاف إلى purchase_price لسطر البيع فتدخل COGS والقيد تلقائيًا
- docs/RESTAURANT_GUIDE.md (جديد) — دليل تشغيل المطعم الكامل: الإعداد، دورات البيع الأربع (POS/طاولات/QR/نادل/توصيل)، KDS، الربحية والرقابة

### فحص الانحدار (NT-0001، وسم TEST-P317)
- دورة كاملة: مكوّن (10 وحدات) + طبق غير مخزّن + وصفة (0.2/وحدة) + إضافة مخزنية (0.1، +100 دج) → بيع ×2:
  - استهلاك مدمج 0.6 (وصفة 0.4 + إضافة 0.2) ✓ مخزون 10→9.4 ✓
  - purchase_price=60 (وصفة 40 + إضافة 20) ✓ — بعد الإصلاح؛ قبله 40 (الفجوة المُثبتة)
  - القيد: 600 مدين 120 / 380 دائن 120 — قيمة المخزون = الكميات ✓
  - الإرجاع: استعادة دقيقة 9.4→10 + قيد عكسي ✓
- ميزان المراجعة متوازن قبل وبعد؛ DLQ=1 قديم (معروف)؛ كل الحاويات Up؛ القرص 42%
- **تنظيف إضافي**: مخلفات TEST-P290 من مرحلة قديمة (JE000008 + معاملة صندوق + رصيد «المال الخاص» −120→0) — أصبح residue=0 فعليًا

### النشر
- خادم فقط (لا واجهة): restart + health=200
...
### التغييرات
- backend/routes/production_routes.py — `GET /production/demand-forecast?days=&cover=`: يجمع الاستهلاك الفعلي للمكونات من أسطر البيع المكلفة بالوصفة (p303، المرتجعات مستثناة) → لكل مكوّن: المستهلك، المتوسط اليومي، المخزون الحالي، أيام التغطية المتبقية، الكمية المقترحة (avg_daily×cover − stock)، التكلفة التقديرية، علم urgent إن كانت التغطية أقل من cover — مرتب بالأكثر إلحاحًا. قراءة فقط
- frontend/src/pages/ProductionPage.js — تبويب رابع «توقع الطلب»: منتقي فترة (14/30/60) + 3 بطاقات ملخص (مكونات/عاجل/تكلفة تقديرية) + قائمة مشتريات مقترحة لكل مكوّن

### الاختبار (NT-0001، وسم TEST-P313، نظّف حتى residue=0)
- سيناريو كامل: جبن (شراء 200، مخزون 3) + بيتزا + وصفة 0.2/قطعة + بيع 10 قطع → forecast: مستهلك 2.0، يومي 0.067، مخزون 1، يكفي 15 يوم، اقتراح 0 ✓؛ cover=30 → اقتراح 1.0 بتكلفة 200 وurgent ✓
- Playwright @1440px: تبويب توقع الطلب يعرض البطاقات والسطر الصحيح ✓ (لقطة p313_forecast.png)
- حذف البيع أعاد مخزون الجبن إلى 3؛ قيدا اليومية (بيع+حذف، متوازنان، COGS 400 من الوصفة) حُذفا؛ residue=0 (مبيعات/منتجات/وصفات/قيود/outbox) + features_override={}

### النشر
- Release 20260826_135755 — chunk 6267.a0623b3f (forecast-tab-trigger مؤكدة)؛ Backend restart + health 200

## 2026-08-26 — p312: واجهة النادل (الخطة ج، المرحلة 2)

### التغييرات
- frontend/src/pages/WaiterPage.js (جديد) — واجهة نادل موبايل-أولاً: شبكة طاولات (فارغة/مشغولة—إلحاق) → شاشة طلب بأصناف كبيرة + حوار إضافات (modifiers) → إرسال للمطبخ عبر POST /restaurant/kitchen-orders بأسعار ولقطات product_id — بلا دفع (الدفع عند الكاشير عبر «تحميل الطلب للسلة» من p311)
- App.js — مسار /waiter (lazy، ProtectedRoute featureKey="restaurant")؛ Layout.js — عنصر «واجهة النادل» في قسم المطعم
- إصلاح أثناء الاختبار: زر «إرسال للمطبخ» كان مغطًّى بشريط التنقل السفلي على الموبايل (z-40 فوق z-20) — رُفع إلى bottom-20 على الموبايل مع بقاء الديسكتوب bottom-4 (pixel-identical @1440px)

### الاختبار (NT-0001، وسم TEST-P312، نظّف حتى residue=0)
- Playwright موبايل 430px: اختيار طاولة → إضافة صنف → إرسال → POST kitchen-orders 200 → عودة لشاشة الطاولات ✓
- طلب المطبخ أُنشئ بحالة pending وبالسعر الخادمي (300) + حدث kitchen_order.created في Redis ✓
- التنظيف: الطلب + الطاولة + منتج الاختبار + outbox/processed_events + أحداث الستريم + استعادة features_override={} ✓

### النشر
- Release 20260826_134237 — main + chunk 3430.9c18bf79 (waiter-page مؤكدة في الحزمة المنشورة)
...
### التغييرات
- frontend/src/pages/TablesMapPage.js (جديد) — خريطة طاولات حية: شبكة بطاقات مجمّعة بالمناطق (فارغة خضراء/مشغولة كهرمانية/متأخرة ≥15د حمراء)، مؤقت الطلب النشط، حوار تفاصيل الطاولة (أسطر الطلب + إنهاء وتحرير + حذف)، إضافة طاولة (اسم/مقاعد/منطقة)، تحديث لحظي SSE + استطلاع 15 ث
- frontend/src/App.js — route `/tables` (featureKey restaurant)؛ Layout.js — عنصر «خريطة الطاولات» في قسم المطعم
- frontend/src/pages/POSPage.js — زر «تقسيم الفاتورة» في قائمة مهام POS (يظهر لميزة restaurant فقط): N حصص (2-6)، تقسيم متساوٍ بالكميات الكسرية أو توزيع الأسطر يدويًا، كل حصة = فاتورة مستقلة مدفوعة نقدًا بنسبتها من الخصم، وتحرير الطاولة تلقائيًا بالفاتورة الأولى
- **إصلاح خلل (p310b)**: التقسيم كان يرسل نفس `saleCode` المجلوب مسبقًا لكل الحصص → DuplicateKeyError على فهرس p257_unique_code فتُنشأ حصة واحدة فقط. الإصلاح مزدوج: الفرونتند يجلب كودًا جديدًا لكل حصة (k>0 مع fallback مُلحق)، والباكند يستبدل أي كود عميل مكرر بكود ذري جديد قبل الإدراج

### الاختبار (NT-0001، وسم TEST-P310، residue=0)
- Playwright @1440px: /tables إضافة طاولة → بطاقة → حذف ✓؛ POS: بحث → إضافة ×2 → تقسيم متساوٍ → فاتورتان BV0045/26 وBV0046/26 (500 دج لكل، كمية 1 لكل، المخزون 100→98 مرة واحدة) ✓
- قبل الإصلاح: فشل مُعاد إنتاجه (حصة واحدة + E11000 في السجلات) → بعد الإصلاح نجاح كامل
- التنظيف: حذف البيعين الاختباريين عبر API (استعادة المخزون 98→100) + حذف قيود اليومية المتوازنة (sale+delete، صافي صفر) + المنتج الاختباري + مرايا outbox/processed_events/nt:events + استعادة features_override={} ✓

### النشر
- Release 20260826_122554 — main.f7655525.js + chunk 977.0033d9fd (علامة tables-map-page مؤكدة)
- Backend: restart + health 200

## 2026-08-26 — p309: وجبات Combo (الخطة ب، المرحلة 3)

### التصميم
وجبة Combo = منتج حزمة قابل للبيع (is_non_stockable، سعر موحد) + وصفة مكوّناتها منتجات قابلة للبيع — يعيد استخدام محرك الوصفات من p303 كما هو: البيع يستهلك مكونات الكومبو من مخزونها ويكلّف السطر بتكلفتها الحقيقية، والإرجاع/الحذف يستعيد بالضبط. لا تغيير باكند.

### التغييرات
- frontend/src/pages/ProductionPage.js — زر «وجبة Combo» بجانب «وصفة جديدة»: معالج (اسم + سعر الوجبة + مكوّنان فأكثر بكميات) مع معاينة حية لتكلفة المكونات الحالية، ينشئ المنتج ثم الوصفة في خطوتين

### الاختبار (NT-0001، وسم TEST-P309، residue=0)
- API: إنشاء كومبو (مشروب 60 + ساندويتش 150، سعر 400) → unit_cost=210 ✓؛ بيع 1 → مخزون المشروب 20→19 والساندويتش 15→14، تكلفة السطر 210، consumption موثق ✓؛ إرجاع → المخزونان عادا 20/15 ✓
- Playwright: معالج الكومبو UI — معاينة التكلفة 210 دج + إنشاء منتج ووصفة من الواجهة ✓
- ملاحظة: اختبار أول فاشل بسبب قراءة معرفات خاطئة في سكربت الاختبار (سطرين في ملف) — حُذف البيع الخاطئ وأعيد بشكل صحيح
- التنظيف: sales/journal/transactions/recipes/products/outbox/Redis = 0، الصندوق -840، features_override استُعيد {}

### النشر
- Release 20260826_102442 — main.a31c3f97.js + chunk 6267.1492c3b3 (علامة combo-dialog مؤكدة)
...
### التغييرات
- backend/routes/production_routes.py (+3 مسارات):
  - `POST /production/waste` — تسجيل هالك المكوّنات: خصم المخزون + سجل waste_log بالتكلفة + قيد يومية تلقائي (مدين 610 مصاريف / دائن 380 مخزون، idempotent عبر auto_entry_unique)
  - `GET /production/waste?days=` — سجل الهالك بالفترة
  - `GET /production/food-cost-report?days=` — لكل طبق: مباع/إيراد/تكلفة/FC%/هامش (من أسطر البيع المكلفة بالوصفة منذ p303؛ المرتجعات مستثناة) + الاستهلاك النظري لكل مكوّن مع مخزونه الحالي + ملخص (إيراد، تكلفة، FC%، هالك ونسبته)
- frontend/src/pages/ProductionPage.js — تبويب ثالث «تكلفة الطعام»: منتقي فترة (7/30/90) + 4 بطاقات ملخص + بطاقة ربحية لكل طبق + بطاقة الاستهلاك النظري للمكونات + سجل الهالك + حوار تسجيل هالك (اختيار مكوّن + كمية + سبب)

### الاختبارات (NT-0001، وسم TEST-P305، 21/21)
- بيع 4 أطباق (تكلفة 50/طبق) + إرجاع بيع طبق + هالك وحدتين @100:
  - المخزون بالضبط 45.5 (50−2−0.5−2) ✓
  - قيد الهالك: 610 مدين 200 / 380 دائن 200 ✓
  - التقرير: إيراد 2000 (المرتجع مستثنى) ✓ تكلفة 200 ✓ FC=10% ✓ هالك 200 (10%) ✓ هامش الطبق 1800 ✓ استهلاك نظري 2.0 وحدة ✓
- بقايا=0 (شمل عكس مرايا 610/380 يدوياً لأن قيد الهالك بلا عكسي) ✓ الصندوق عاد لـ -840 بالضبط ✓
- ast.parse + esbuild ✓ health 200 (70ث) ✓ release 20260826_013850 ✓ Playwright: التبويب والبطاقات الأربع وزر الهالك تعمل بلا أخطاء (فعّلنا ميزة production على NT-0001 مؤقتاً ثم أعدناها إلى {}) ✓

### اكتملت الخطة أ (p303-p305) — التالي: الخطة ب (واجهة المطعم المتخصصة)

## 2026-08-26 — p304: التكلفة الحية للوصفات + هامش ربح الطبق (الخطة أ للمطاعم، المرحلة 2)

### التغييرات
- backend/routes/production_routes.py — GET /production/recipes يحسب الآن لكل وصفة: live_unit_cost من أسعار شراء المكوّنات **الحالية** (المشتريات تحدّث purchase_price فوراً — لا كاش يفسد)، dish_price (سعر بيع الطبق)، margin و margin_pct و food_cost_pct؛ السقوط إلى unit_cost المخزّن فقط عند حذف منتج مكوّن
- frontend/src/pages/ProductionPage.js — بطاقة الوصفة تعرض: التكلفة الحية + سعر البيع + الهامش% (أخضر/أحمر حسب الإشارة) + Food Cost% (data-testid للتحقق: live-cost-*/margin-*)

### الاختبارات (NT-0001، وسم TEST-P304، 12/12)
- وصفة بطبق 900 دج ومكوّن 50×0.2: live=10، هامش 890 (98.9%)، FC=1.1% ✓
- **إثبات الحيوية**: رفع سعر شراء المكوّن 50←200 → التكلفة الحية 10←40 فوراً والمخزّن يبقى 10 ✓
- بقايا=0 ✓ ast.parse + esbuild ✓ health 200 (75ث) ✓ build+deploy (release 20260826_011348، العلامة في chunk 6267 — التقسيم الكسول) ✓ Playwright /production بلا أخطاء ✓

## 2026-08-26 — p303: ربط البيع بالوصفة — أرباح المطاعم حقيقية الآن (الخطة أ للمطاعم، المرحلة 1)

### المشكلة
- أطباق المطاعم (منتجات غير مخزّنة بسعر شراء 0) كانت تُباع بلا خصم مكوّنات وبتكلفة صفر → COGS=0 → ربح وهمي 100% ولا جرد للمواد الأولية

### التغييرات (backend/services/application/sales_service.py)
- عند إنشاء البيع: كل عنصر له وصفة في recipes (محرك BOM من p188) → حساب تكلفة الوحدة لحظياً من أسعار شراء المكوّنات الحالية (مع سقوط إلى unit_cost المخزّن في الوصفة)، وخصم الكميات المستهلكة من مخزون المكوّنات داخل نفس معاملة Mongo — **الخصم لا يمنع البيع** (واقع المطابخ: المكوّن قد يسلب مؤقتاً حتى يصل شراء) بعكس منتجات التجزئة التي تبقى صارمة
- سعر شراء السطر يُضبط على تكلفة الوصفة (مرجعية) → قيد COGS (600) يتدفق تلقائياً عبر المحاسبة الآلية الموجودة بلا أي تعديل عليها
- السطر يخزّن recipe_consumption (موقّعة بالإشارة) → الإرجاع والحذف يستعيدان المكوّنات بدقة (سطور الإرجاع السالبة داخل البيع تعمل تلقائياً بنفس المنطق)
- لا بوابة ميزة: أي مستأجر أنشأ وصفة يستفيد فوراً

### الاختبارات (NT-0001، وسم TEST-P303، 22/22)
- وصفة بيتزا (عجين 0.2 + جبن 0.1): تكلفة محسوبة 90 ✓
- بيع بيتزتين: خصم 0.4 عجين و0.2 جبن (999.6/499.8) ✓ وسطر البيع بتكلفة 90 ✓ وقيد 600 مدين 180 (واقعي) ✓
- إرجاع: استعادة كاملة 1000/500 ✓ | حذف (سبب إلزامي): استعادة كاملة ✓
- تراجع: منتج عادي يُخصم كما قبل ✓ البيع فوق المخزون ما زال مرفوضاً 400 ✓ الصندوق عاد للقيمة المرجعية بالضبط (-840) ✓
- بقايا=0: sales/journal_entries/products/recipes/notifications في قاعدة المستأجر + outbox/processed_events في الرئيسية ✓
- ast.parse ✓ health 200 (55ث) ✓ Playwright: /pos و/production تعملان (أخطاء CSP للخطوط قديمة سابقة للمرحلة) ✓

### التالي في الخطة أ
- p304: تكلفة الوصفة الحية تتحدث مع كل شراء (متوسط موزون) + هامش ربح كل طبق
- p305: تقرير Food Cost % + ربحية الأطباق + الهالك
...
### التغييرات
- backend/routes/whatsapp_routes.py: PUT /whatsapp/config يشفّر access_token بـ AES-256-GCM (crypto_fields)؛ القناع المرتجع من GET ("EAA...") لا يُحفظ بل يُبقي القيمة المخزنة؛ GET يفك التشفير ثم يقنّع (توافق رجعي مع النصوص القديمة)
- frontend/src/pages/WhatsAppPage.js: حوار الإعدادات — حقلا Phone Number ID و Access Token استُبدلا بملاحظة توجّه إلى مركز التكاملات (wa-page-hub-note/link)؛ إصلاح تلف ترميز قديم في تسمية «الرسالة»
- ملاحظة: التبويبات (WhatsAppTab/EmailTab) حُوّلت لملاحظات المركز في p288، و/whatsapp/settings مشفّر منذ p272

### الاختبارات
- PUT بمفتاح حقيقي → يُخزَّن "v1...." مشفّراً ✓ ; GET → مقنّع ✓ ; PUT بالقناع → القيمة لا تتغير ✓ ; مسح → "" ✓ (بقايا=0)
- Playwright: ملاحظة المركز ظاهرة، لا حقول سرّية في الحوار، تسمية «الرسالة» سليمة ✓
- build: main.fe6e091c.js → نُشر (release 20260825_015617)

### بنية تحتية
- حظر Hostinger لعنوان بيئة العمل على SSH → الوصول الآن عبر Tailscale (srv1774015 = 100.76.183.40)
...
## p279 — خطة الجوال: المرحلة هـ (اللمسات الأخيرة) (2026-08-24)

### التغييرات
- ui/dialog.jsx: الحوارات على الجوال p-4 بدل p-6 + max-h-[92dvh] overflow-y-auto (لا حوار يتجاوز الشاشة)؛ سطح المكتب دون تغيير (sm:p-6، وأي max-h خاص بالحوار يتغلب عبر tailwind-merge).
- index.css: قاعدة media query — حقول input/select/textarea بخط 16px على الجوال (<768px) لمنع التكبير التلقائي في iOS.
- POSCart.js + EcomManualOrderDialog.js: inputMode=numeric/decimal على 10 حقول رقمية (كمية/سعر/خصم/مدفوع/مختلط/طلب يدوي) — لوحة مفاتيح رقمية مباشرة على الجوال.

### الاختبارات (headless على nt-commerce.net)
...
## p287 — مركز التكاملات والمفاتيح الموحّد (Integrations Hub) (2026-08-24)

**طلب المالك:** واجهة واحدة تجمع كل مفاتيح API والويب هوكات ومفاتيح الربط في النظام، مع شروحات جلب كل مفتاح، وتفعيل تلقائي للخدمة عند وضع المفتاح الصحيح، وصلاحيات إدارة مركزية.

**Backend (`routes/integrations_hub_routes.py`، مسجّل في `_AUTO_REG_MODULES`):**
- سجلّ موحد REGISTRY بـ **18 تكاملاً** في 5 فئات: الشحن (ياليدين، Guepex، ZR، مايسترو، Ecotrack، NOEST) — التواصل (واتساب، فيسبوك، ماسنجر، إنستغرام) — المراسلة (تيليغرام، بوابة SMS، SendGrid) — المتاجر (WooCommerce، Shopify) — أدوات المطورين (ويب هوك الشحن، مصادر الاستقبال، مفاتيح API كروابط).
- المركز **واجهة موحِّدة فوق التخزين القائم** (ecom_integrations لكل قناة، email_integration_settings، ecom_sms_settings، woocommerce_settings) — لا تخزين موازٍ، فكل الصفحات الحالية تستمر بالعمل.
- `GET /api/integrations-hub/catalog` — الحالة الحية لكل تكامل (مهيأ/مفعّل/آخر فحص) + مفاتيح مقنّعة (••••last4 بعد فك التشفير) + رابط Webhook لكل ناقل مدعوم (نفس توكنات p284، إنشاء كسول).
- `POST /connect` — تحقق من الحقول المطلوبة ← حفظ مشفّر (AES-256-GCM) ← **فحص اتصال حقيقي** (ياليدين/Shopify عبر خدماتهما، Meta عبر Graph API، تيليغرام getMe، SendGrid /v3/scopes، WooCommerce /wc/v3) ← **التفعيل التلقائي عند النجاح فقط** (is_active=true؛ عند الفشل تُحفظ المفاتيح ويبقى موقوفاً مع رسالة الرفض) + hub_last_test على المستند.
- `POST /test` و `/disconnect` (إيقاف مع إبقاء المفاتيح). تدقيق وصول main_db.hub_audit لكل connect/disconnect.
- الصلاحيات: الكتالوج require_tenant، التعديلات get_tenant_admin.

**Frontend (`pages/IntegrationsHubPage.js`، مسار `/integrations`، عنصر قائمة «مركز التكاملات» بقسم الإعدادات):**
- بطاقات مجمّعة بالفئة مع شارة حالة (مفعّل/خطأ في المفتاح/موقوف/غير مهيأ/صفحة مخصصة) + ملخص مفاتيح مقنّع + شريط Webhook مع نسخ وعدّاد أحداث.
- حوار ربط لكل تكامل: دليل «كيف أجلب المفتاح؟» مرقّم + رابط مباشر، حقول سرية بإظهار/إخفاء، زر «حفظ واختبار»، رسالة نتيجة فورية، كتلة Webhook مع التعليمات.
- بحث فوري + تصميم متجاوب (1/2/3 أعمدة).
...
## p289 — سير عمل إرسال الطلبات الموحّد: يدوي/تلقائي + تذكير الزبون + حالة الناقل على الطلب (2026-08-24)

**السياق:** عمل جلسة موازية — رُوجع واختُبر وأُصلح وثُبّت (قاعدة الجلسات الموازية). يستكمل طلب المالك: أيقونتا معالجة لكل طلب (يدوي: نسخ بيانات الزبون + تعليم مسجَّل/غير مسجَّل؛ تلقائي: الناقلون المربوطون فقط ← إنشاء البوليصة ← طباعتها)، حالة الناقل على الطلب، وتذكير SMS/WhatsApp.

**Backend (`routes/ecom/order_dispatch_routes.py` جديد، مسجَّل في main.py):**
- `GET /ecom/dispatch/couriers` — شركات الشحن المربوطة والمفعّلة فقط (is_active) مع supports_auto/creates_label.
- `POST /ecom/orders/{id}/dispatch` — إرسال تلقائي: يتحقق أن الناقل مربوط ← create_label (حقيقي لياليدين، بديل تتبع داخلي للباقي) ← fulfillment{mode:auto} + status_history.
- `POST /ecom/orders/{id}/manual-registration` — تسجيل يدوي: fulfillment{mode:manual, registered_with_courier} + courier + status_history.
- `POST /ecom/orders/{id}/remind` — تذكير الزبون SMS (خصم ذري من رصيد المحفظة + ecom_sms_logs) أو WhatsApp (graph API + whatsapp_logs)؛ رسالة افتراضية بالكود والمبلغ ورقم التتبع؛ يرفض بلا هاتف/بوابة/مفاتيح برسائل توجّه لمركز التكاملات.
- **إصلاح أثناء المراجعة:** رسالة النتيجة عند فشل الإنشاء الحقيقي لدى ناقل يدعمه (ياليدين بمفاتيح مرفوضة) كانت توحي بنجاح ضمني — أصبحت صراحة: «⚠️ تعذّر الإنشاء الحقيقي لدى الناقل — راجع المفاتيح في مركز التكاملات. سُجّلت بوليصة تتبع مؤقتة».

**Frontend:**
- `pages/ecom/EcomOrderProcessDialog.js` جديد: اختيار يدوي/تلقائي؛ اليدوي: 9 حقول زبون بأزرار نسخ + زرا «مسجَّل في الشركة/غير مسجَّل»؛ التلقائي: ناقلون مربوطون فقط (process-no-couriers يربط بمركز التكاملات) ← تأكيد ← نتيجة البوليصة (تتبع + طباعة)؛ شريط «المعالجة الحالية».
- `pages/ecom/EcomHubPage.js`: عمود «الشحن» (شارة الناقل + ⚡تلقائي/✋يدوي + مسجَّل/غير مسجَّل + رقم التتبع + courier_last_status)؛ أزرار لكل طلب: معالجة 🚚 (process-order-)، تذكير 💬 (remind-order- بحوار SMS/WhatsApp)، طباعة البوليصة 🖨 (print-label-) إن وُجدت.

**الاختبارات (NT-0001، TEST-P289):** couriers يرجع الناقل المربوط فقط ✓؛ dispatch تلقائي (مفاتيح مزيفة ← سقوط mock رشيق + بوليصة YAL-* وtracking) ✓؛ ناقل غير مربوط 400 ✓؛ بلا ناقل 400 ✓؛ manual registered=true ✓؛ طلب مجهول 404 ✓؛ قناة تذكير خاطئة 400 ✓؛ SMS بلا بوابة 400 ✓؛ WhatsApp غير مفعّل 400 ✓؛ بلا هاتف 400 ✓؛ تثبت الحقول في Mongo (fulfillment/tracking/courier) ✓؛ E2E: 14 زر معالجة، حوار يدوي (18 صف نسخ + الزران)، قائمة الناقلين تعرض المربوط فقط، إرسال تلقائي ← نتيجة بوليصة + طباعة، حوار التذكير + توست حارس SMS، شارات الشحن على الصفوف ✓؛ صفر أخطاء console غير مسبوقة. **بقايا = 0** (orders/labels/integrations/sms/wa/notifications/outbox).

**النشر:** الحزمة الحية main.b4e313c8.js + chunk 1912 (بناء الجلسة الموازية)؛ إصلاح الرسالة backend فقط (لا إعادة بناء).


## p290 — ربحية كل منتج: تغليف + إعلانات منسوبة مباشرة + رابح/خاسر + سجل المنتج (2026-08-24)

**طلب المالك:** ربحية كل منتج (مُسلَّم/مُرجَع، كل التكاليف والخسائر في سجل المنتج — تغليف وإرجاعات وإعلانات تُسحب من تبويب الإعلانات — صافي الربح، مقارنة الرابح/الخاسر).

**Backend:**
- `routes/ecom/analytics_routes.py` — توسيع `GET /ecom/analytics/product-pnl`: +حقول delivered/returned في الإخراج، +تخصيص packaging_cost لكل منتج بحصته من الطلب (تغليف المُسلَّم كلفة وتغليف المُرجَع خسارة)، +خسارة الإرجاع الكاملة (شحن + رسم الإرجاع من تكامل الناقل return_fee + التغليف)، +نسبة إعلانية مباشرة (expense.product_id → المنتج) مع بقاء التوزيع النسبي لغير المربوط، +winners/losers (أعلى 3 ربحاً وأدنى 3 خسارة)، +ad_attributed.
- جديد `GET /ecom/analytics/product-history?key=&days=` — سجل منتج واحد: كل طلب ظهر فيه (كود/تاريخ/حالة/زبون/تتبع) مع حصته من الإيراد/التكلفة/التغليف/الشحن/خسارة الإرجاع + إعلانات المنتج المربوطة + إجماليات.
- `routes/expenses_routes.py` — ExpenseCreate: +product_id/product_name اختياريان.

**Frontend:**
- `pages/ecom/EcomAnalyticsPage.js` — بطاقتا «🏆 المنتجات الرابحة» و«📉 المنتجات الخاسرة» فوق الجدول؛ أعمدة جديدة: مُسلَّم/مُرجَع/تغليف؛ ملاحظة توزيع الإعلانات (إجمالي/مربوط مباشرة)؛ زر «السجل» لكل منتج يفتح حواراً (إجماليات + إعلانات المنتج + جدول الطلبات بالحالات والتكاليف).
- `pages/ecom/EcomAdsTab.js` — قائمة «المنتج المُعلَن عنه» اختيارية في نموذج الصرف الإعلاني (ad-product-select) — تُرسل product_id/product_name مع المصروف.
...
## p296 — إعدادات مفتاح الذكاء الاصطناعي من لوحة السوبر أدمن (2026-08-25)

**طلب المالك:** «من أين أضع مفتاح API الخاص بالذكاء الاصطناعي كي تستعمله المنصة» — كان المفتاح يُضبط فقط عبر متغيرات البيئة في السيرفر بلا واجهة.

**Backend:**
- `services/ai/platform_ai_settings.py` (جديد): مصدر فعّال للإعداد = قاعدة البيانات أولاً (main_db.ai_platform_settings، المفتاح مشفّر AES-256-GCM عبر crypto_fields) ثم متغيرات البيئة كاحتياط؛ كاش داخلي TTL 30 ثانية يُبقي القراءات المتزامنة صادقة بلا ضغط على Mongo، ويُحدَّث عند كل استدعاء LLM ويُبطل فور الحفظ.
- `services/ai/openai_llm.py` + `services/ai/llm_service.py`: get_api_key/_client/_model/_get_openai_client/send_message كلها تقرأ الإعداد الفعّال بدل env مباشرة (مع تحديث الكاش في المسارات غير المتزامنة).
- `routes/saas/ai_billing_routes.py` (+3 مسارات super admin): `GET /saas/ai-settings` (حالة + مفتاح مقنّع + مصدر كل قيمة)، `PUT /saas/ai-settings` (حفظ مشفّر؛ الصدى المقنّع «…» يُبقي المفتاح؛ clear_key=true يحذف مفتاح الواجهة ويعيد الاعتماد على السيرفر)، `POST /saas/ai-settings/test` (اتصال حي بأقصر استدعاء، مع fallback max_tokens→max_completion_tokens لعائلة gpt-5).

**Frontend (`AiBillingCard.js`):** بطاقة «مفتاح الذكاء الاصطناعي للمنصة» أعلى تبويب فوترة الذكاء: شارة الحالة (مضبوط/غير مضبوط + المصدر: الواجهة/السيرفر)، حقول المفتاح (مقنّع)/نقطة النهاية/النموذج، حفظ + «اختبار الاتصال» بنتيجة ملونة، وزر حذف مفتاح الواجهة.

**الاختبارات:** دورة كاملة عبر API — اختبار بمفتاح env (نجاح حي: Gemini «Confirmed.») → مفتاح DB وهمي يغلب على env (رفض المزوّد INVALID_ARGUMENT يثبت الأولوية) → الصدى المقنّع يُبقي المفتاح → clear_key يعيد env → اختبار ناجح مجدداً. E2E: البطاقة ظاهرة، الشارة «مضبوط — المصدر: السيرفر»، النموذج gemini-3.1-flash-lite، واختبار الاتصال من الواجهة ناجح. **بقايا = 0** (وثيقة الإعداد التجريبية حُذفت؛ لا قيود استهلاك لمسار الاختبار).

**النشر:** release 20260825_000806، main.8afde5ad.js.
