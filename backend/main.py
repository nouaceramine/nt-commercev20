...
async def startup_event():
    ...
    try:
        ...
        _CODE_INDEXES = [
            ...
            ("sales", "code"),  # p283: unique now — MP00001x2 test residue removed
        ]
        for _d in _dbs2:
            ...
            # p283: sales.code now unique (in _CODE_INDEXES); keep plain index only
            # if the unique build failed on this DB (e.g. legacy dup) so search stays fast.
            try:
    ...
    try:
        ...
        await db.ecom_import_batches.create_index("created_at")
        await db.ecom_orders.create_index("import_batch_id")
        await db.ecom_referrals.create_index("id", unique=True)
        await db.ecom_referrals.create_index("code", unique=True)
        await db.ecom_referral_rewards.create_index("id", unique=True)
        ...
        await db.ecom_social_sources.create_index("id", unique=True)
        await db.ecom_social_sources.create_index("token", unique=True)
        await db.ecom_social_conversations.create_index("id", unique=True)
