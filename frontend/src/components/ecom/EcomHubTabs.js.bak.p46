...
import { ShoppingBag, Store, Link2, Megaphone, Truck, BarChart3 } from 'lucide-react';
...
export function EcomHubTabs() {
  ...
  const current = HUB_TABS.find(t => t.path === location.pathname)?.key
    || (location.pathname === '/ecom-hub' ? 'orders' : 'orders');
