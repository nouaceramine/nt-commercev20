...
export default function DefectiveGoodsPage() {
  ...
  const severityColor = (s) => ({ high: 'bg-red-500/10 text-red-400 border-red-500/30', medium: 'bg-amber-500/10 text-amber-400 border-amber-500/30', low: 'bg-emerald-500/10 text-emerald-400 border-emerald-500/30' }[s] || 'bg-gray-500/10 text-muted-foreground');
  const statusColor = (s) => ({ pending_inspection: 'bg-amber-500/10 text-amber-400', confirmed_defective: 'bg-red-500/10 text-red-400', not_defective: 'bg-emerald-500/10 text-emerald-400' }[s] || 'bg-gray-500/10 text-muted-foreground');
    ...
    <Layout>
      <div className="p-4 md:p-6 space-y-6" data-testid="defective-goods-page">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h1 className="text-2xl font-bold text-foreground">{isAr ? 'البضائع المعيبة' : 'Produits Défectueux'}</h1>
            <p className="text-sm text-muted-foreground mt-1">{isAr ? 'إدارة ومتابعة المنتجات المعيبة' : 'Gestion des produits défectueux'}</p>
          </div>
        ...
        <div className="flex flex-col md:flex-row gap-3">
          <div className="relative flex-1"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" /><Input placeholder={isAr ? 'بحث...' : 'Rechercher...'} value={search} onChange={e => setSearch(e.target.value)} className="pl-10 bg-card border-border text-foreground" data-testid="defective-search" /></div>
          <Select value={filterStatus} onValueChange={setFilterStatus}><SelectTrigger className="w-48 bg-card border-border text-foreground" data-testid="defective-status-filter"><SelectValue /></SelectTrigger>
        ...
        <div className="space-y-3">
          {loading ? <p className="text-muted-foreground text-center py-8">{isAr ? 'جاري التحميل...' : 'Chargement...'}</p> :
           items.length === 0 ? <p className="text-muted-foreground text-center py-8">{isAr ? 'لا توجد بيانات' : 'Aucune donnée'}</p> :
           items.map(item => (
            <Card key={item.id} className="bg-card border-border hover:border-input transition-colors" data-testid={`defective-item-${item.id}`}>
              <CardContent className="p-4 flex flex-col md:flex-row justify-between items-start md:items-center gap-3">
                <div className="flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="text-xs font-mono text-muted-foreground">{item.defective_number}</span>
                  ...
                  <h3 className="text-foreground font-medium mt-1">{item.product_name}</h3>
                  <p className="text-sm text-muted-foreground">{isAr ? 'الكمية' : 'Qté'}: {item.quantity} | {isAr ? 'التكلفة' : 'Coût'}: {(item.total_cost || 0).toLocaleString()} DA</p>
                </div>
        ...
        <Dialog open={showCreate} onOpenChange={setShowCreate}>
          <DialogContent className="bg-background border-border text-foreground max-w-md">
            ...
            <div className="space-y-4">
              <Input placeholder={isAr ? 'اسم المنتج' : 'Nom du produit'} value={form.product_name} onChange={e => setForm({...form, product_name: e.target.value})} className="bg-card border-border" data-testid="defective-product-name" />
              <div className="grid grid-cols-2 gap-3">
                <Select value={form.defect_severity} onValueChange={v => setForm({...form, defect_severity: v})}><SelectTrigger className="bg-card border-border"><SelectValue /></SelectTrigger>
                ...
                <Input type="number" placeholder={isAr ? 'الكمية' : 'Quantité'} value={form.quantity} onChange={e => setForm({...form, quantity: parseInt(e.target.value) || 0})} className="bg-card border-border" />
              </div>
              <Input type="number" placeholder={isAr ? 'سعر الوحدة' : 'Prix unitaire'} value={form.unit_cost} onChange={e => setForm({...form, unit_cost: parseFloat(e.target.value) || 0})} className="bg-card border-border" />
              <Input placeholder={isAr ? 'الوصف' : 'Description'} value={form.description} onChange={e => setForm({...form, description: e.target.value})} className="bg-card border-border" />
