...
export default function CustomerFamiliesPage() {
  ...
  const handleSave = async (createNew = false) => {
    ...
    try {
      if (editingFamily) {
        await apiClient.put(`/customer-families/${editingFamily.id}`, formData);
        toast.success(language === 'ar' ? 'تم تحديث العائلة بنجاح' : 'Famille mise à jour');
        setShowDialog(false);
      } else {
        await apiClient.post(`/customer-families`, formData);
  ...
  const handleDelete = async () => {
    ...
    try {
      await apiClient.delete(`/customer-families/${deletingFamily.id}`);
  ...
  return (
    <Layout>
      <div className="space-y-6 animate-fade-in" data-testid="customer-families-page">
        {/* Header */}
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold tracking-tight">
              {language === 'ar' ? 'عائلات الزبائن' : 'Familles de clients'}
            </h1>
            <p className="text-muted-foreground mt-1">
              {language === 'ar' ? 'تصنيف وتجميع الزبائن في مجموعات' : 'Catégoriser et regrouper les clients'}
            </p>
          </div>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead>{language === 'ar' ? 'عدد الزبائن' : 'Nb clients'}</TableHead>
                    <TableHead></TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                      ...
                      <TableCell>
                        <div className="flex items-center gap-2 justify-end">
                          ...
                          <Button 
                            ...
                            disabled={family.customer_count > 0}
